...
ChairObjectInterface {
    ...
    protected BenchObject(String textureName, ToolType toolType, Color mapColor, String ... category) {
    ...
    public void addDrawables(List<LevelSortedDrawable> list, OrderableDrawables tileList, Level level, int tileX, int tileY, TickManager tickManager, GameCamera camera, PlayerMob perspective) {
        ...
        if (rotation == 0) {
            drawOptions = texture.initDraw().sprite(2, 3, 32).light(light).pos(drawX, drawY);
        } else if (rotation == 1) {
            drawOptions = texture.initDraw().sprite(0, 1, 32, 64).light(light).pos(drawX, drawY - 32);
        } else if (rotation == 2) {
            drawOptions = texture.initDraw().sprite(3, 1, 32).light(light).pos(drawX, drawY - 32);
            drawOptions2 = texture.initDraw().sprite(3, 2, 32).light(light).pos(drawX, drawY);
        } else {
            drawOptions = texture.initDraw().sprite(1, 0, 32, 64).light(light).pos(drawX, drawY - 32);
        }
    ...
    public void modifyHumanDrawOptions(Level level, int tileX, int tileY, HumanDrawOptions options) {
        int modifiedRotaion = this.getModifiedRotation(level, tileX, tileY);
        options.dir(modifiedRotaion).sprite(6, modifiedRotaion);
    }
    ...
    public Point getMobPosSitOffset(Level level, int tileX, int tileY) {
        ...
        switch (rotation) {
            case 0: {
                return new Point(17, 8);
            }
            case 1: {
                return new Point(22, 16);
            }
            case 2: {
                return new Point(17, 18);
            }
            case 3: {
    ...
    public void drawPreview(Level level, int tileX, int tileY, int rotation, float alpha, PlayerMob player, GameCamera camera) {
        ...
        if (rotation == 0) {
            texture.initDraw().sprite(2, 1, 32).alpha(alpha).draw(drawX, drawY - 64);
            texture.initDraw().sprite(2, 2, 32).alpha(alpha).draw(drawX, drawY - 32);
            texture.initDraw().sprite(2, 3, 32).alpha(alpha).draw(drawX, drawY);
        } else if (rotation == 1) {
            texture.initDraw().sprite(0, 1, 64).alpha(alpha).draw(drawX, drawY - 32);
        } else if (rotation == 2) {
            texture.initDraw().sprite(3, 1, 32).alpha(alpha).draw(drawX, drawY - 32);
            texture.initDraw().sprite(3, 2, 32).alpha(alpha).draw(drawX, drawY);
            texture.initDraw().sprite(3, 3, 32).alpha(alpha).draw(drawX, drawY + 32);
        } else {
    ...
    public static int[] registerBench(String stringID, String textureName, ToolType toolType, Color mapColor, float brokerValue, String ... category) {
        int bdi;
        BenchObject buo = new BenchObject(textureName, toolType, mapColor, category);
        Bench2Object bdo = new Bench2Object(textureName, toolType, mapColor, category);
    ...
    public static int[] registerBench(String stringID, String textureName, Color mapColor, float brokerValue, String ... category) {
        return BenchObject.registerBench(stringID, textureName, ToolType.ALL, mapColor, brokerValue, category);
    }
