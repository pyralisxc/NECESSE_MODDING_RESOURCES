...
class DoubleBed1HeadObject
extends DoubleBedHeadBaseObject {
    protected int foot1ID;
    protected int head2ID;
    protected int foot2ID;

    protected DoubleBed1HeadObject(String textureName, ToolType toolType, Color mapColor, String ... category) {
    ...
    public MultiTile getMultiTile(int rotation) {
        return new SidedRotationMultiTile(1, 1, 2, 2, rotation, true, this.foot2ID, this.foot1ID, this.head2ID, this.getID());
    }
    ...
    public void modifyHumanDrawOptions(Level level, int tileX, int tileY, HumanDrawOptions options) {
        ...
        switch (rotation) {
            ...
            case 1: {
                ...
                options.mask(this.maskTextures[1], -22 - yOffset, -14);
                break;
            }
            case 2: {
    ...
    public void addDrawables(List<LevelSortedDrawable> list, OrderableDrawables tileList, Level level, int tileX, int tileY, TickManager tickManager, GameCamera camera, PlayerMob perspective) {
        ...
        if (rotation == 0) {
            options.add(baseTexture.initDraw().sprite(3, 5, 32).light(light).pos(drawX, drawY));
        } else if (rotation == 1) {
            options.add(baseTexture.initDraw().sprite(0, 5, 32).light(light).pos(drawX, drawY));
        } else if (rotation == 2) {
            options.add(baseTexture.initDraw().sprite(2, 0, 32).light(light).pos(drawX, drawY - 32));
            options.add(baseTexture.initDraw().sprite(2, 1, 32).light(light).pos(drawX, drawY));
        } else {
            options.add(baseTexture.initDraw().sprite(1, 0, 32).light(light).pos(drawX, drawY - 32));
            options.add(baseTexture.initDraw().sprite(1, 1, 32).light(light).pos(drawX, drawY));
        }
    ...
    public void drawPreview(Level level, int tileX, int tileY, int rotation, float alpha, PlayerMob player, GameCamera camera) {
        ...
        if (rotation == 0) {
            baseTexture.initDraw().sprite(3, 5, 32).alpha(alpha).draw(drawX, drawY);
        } else if (rotation == 1) {
            baseTexture.initDraw().sprite(0, 5, 32).alpha(alpha).draw(drawX, drawY);
        } else if (rotation == 2) {
            baseTexture.initDraw().sprite(2, 0, 32).alpha(alpha).draw(drawX, drawY - 32);
            baseTexture.initDraw().sprite(2, 1, 32).alpha(alpha).draw(drawX, drawY);
        } else {
            baseTexture.initDraw().sprite(1, 0, 32).alpha(alpha).draw(drawX, drawY - 32);
            baseTexture.initDraw().sprite(1, 1, 32).alpha(alpha).draw(drawX, drawY);
        }
    }
}
