...
public class UpgradeStationObject
extends GameObject {
    ...
    public void loadTextures() {
        super.loadTextures();
        this.texture = ObjectDamagedTextureArray.loadAndApplyOverlay((GameObject)this, "objects/upgradestation");
    }
    ...
    public void addDrawables(List<LevelSortedDrawable> list, OrderableDrawables tileList, Level level, int tileX, int tileY, TickManager tickManager, GameCamera camera, PlayerMob perspective) {
        ...
        final TextureDrawOptionsEnd drawOptions = texture.initDraw().light(light).pos(drawX + 16 - texture.getWidth() / 2, drawY - texture.getHeight() + 32 + 4);
    ...
    public void drawPreview(Level level, int tileX, int tileY, int rotation, float alpha, PlayerMob player, GameCamera camera) {
        ...
        texture.initDraw().alpha(alpha).draw(drawX + 16 - texture.getWidth() / 2, drawY - texture.getHeight() + 32 + 4);
    }

    @Override
    public ListGameTooltips getItemTooltips(InventoryItem item, PlayerMob perspective) {
        ListGameTooltips tooltips = super.getItemTooltips(item, perspective);
        tooltips.add(Localization.translate("itemtooltip", "upgradestationtip"), 400);
        return tooltips;
    }
    ...
    public void interact(Level level, int x, int y, PlayerMob player) {
        ...
        if (level.isServer()) {
            UpgradeStationContainer.openAndSendContainer(ContainerRegistry.UPGRADE_STATION_CONTAINER, player.getServerClient(), level, x, y);
        }
    }

    @Override
    public ObjectEntity getNewObjectEntity(Level level, int x, int y) {
        return new UpgradeStationObjectEntity(level, x, y);
    }
