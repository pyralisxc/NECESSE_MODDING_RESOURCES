...
public class TrialEntranceObject
extends GameObject {
    ...
    public void addDrawables(List<LevelSortedDrawable> list, OrderableDrawables tileList, Level level, int tileX, int tileY, TickManager tickManager, GameCamera camera, PlayerMob perspective) {
        ...
        final TextureDrawOptionsEnd options = this.texture.initDraw().sprite(0, 0, 32).color(color).light(light).pos(drawX, drawY);
    ...
    public void drawPreview(Level level, int tileX, int tileY, int rotation, float alpha, PlayerMob player, GameCamera camera) {
        ...
        GameTile tile = level.getTile(tileX, tileY);
        Color color = tile.getMapColor(level, tileX, tileY);
        this.texture.initDraw().sprite(0, 0, 32).color(color).light(light).alpha(alpha).draw(drawX, drawY);
    }
    ...
    public ObjectEntity getNewObjectEntity(Level level, int x, int y) {
        return new TrialEntranceObjectEntity(level, x, y);
    }
}
