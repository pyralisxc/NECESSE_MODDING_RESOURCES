...
public class DiningSetTableDecorationObject
extends GameObject {
    ...
    public DiningSetTableDecorationObject(String textureName, ToolType toolType, Color mapColor, int decorationWidth, int decorationHeight, int holderDrawXOffset, int holderDrawYOffset) {
    ...
    public DiningSetTableDecorationObject(String textureName, Color mapColor, int decorationWidth, int decorationHeight, int holderDrawXOffset, int holderDrawYOffset) {
        this(textureName, ToolType.ALL, mapColor, decorationWidth, decorationHeight, holderDrawXOffset, holderDrawYOffset);
    }

    public DiningSetTableDecorationObject(String textureName, Color mapColor, int decorationWidth, int decorationHeight) {
        this(textureName, mapColor, decorationWidth, decorationHeight, 0, 0);
    }

    public DiningSetTableDecorationObject setCustomLocalization(GameMessage name) {
        this.customLocalization = name;
        return this;
    }

    public DiningSetTableDecorationObject setCustomLocalization(String category, String key) {
        return this.setCustomLocalization(new LocalMessage(category, key));
    }

    public DiningSetTableDecorationObject setCustomLocalization(String key) {
    ...
    public void addLayerDrawables(List<LevelSortedDrawable> list, OrderableDrawables tileList, Level level, int layerID, int tileX, int tileY, TickManager tickManager, GameCamera camera, PlayerMob perspective) {
        ...
        final TextureDrawOptionsEnd options = texture.initDraw().sprite(rotation, 0, 32, texture.getHeight()).light(light).pos(drawX, drawY - texture.getHeight() + 32 + yOffset);
    ...
    public void drawPreview(Level level, int tileX, int tileY, int rotation, float alpha, PlayerMob player, GameCamera camera) {
        ...
        texture.initDraw().sprite(rotation, 0, 32, texture.getHeight()).alpha(alpha).draw(drawX, drawY - texture.getHeight() + 32 + yOffset);
    }
    ...
    public ArrayList<ObjectPlaceOption> getPlaceOptions(Level level, int levelX, int levelY, PlayerMob playerMob, int playerDir, boolean offsetMultiTile) {
        ...
        if (belowObject instanceof DecorationHolderInterface && belowObject.getHoverHitboxes(level, 0, testTileX, testTileY + 1).stream().anyMatch(hitbox -> hitbox.contains(levelX, levelY))) {
            ...
            placeOptions.add(0, new ObjectPlaceOption(tileX, tileY + 1, this, playerDir, false));
        }
        return placeOptions;
    }
