...
extends GameObject {
    ...
    public void loadTextures() {
        super.loadTextures();
        this.texture = this.texturePath == null ? ObjectDamagedTextureArray.loadAndApplyOverlay((GameObject)this, "objects/paintings/" + this.getStringID()) : ObjectDamagedTextureArray.loadAndApplyOverlay((GameObject)this, "objects/" + this.texturePath);
    }
    ...
    public void addLayerDrawables(List<LevelSortedDrawable> list, OrderableDrawables tileList, Level level, int layerID, int tileX, int tileY, TickManager tickManager, GameCamera camera, PlayerMob perspective) {
        ...
        if (rotation == 0) {
            sortY = 32;
            options.add(texture.initDraw().sprite(0, 2, 32, 32).light(light).pos(drawX, drawY + 8));
        } else if (rotation == 1) {
            sortY = 24;
            options.add(texture.initDraw().sprite(0, 3, 32, 32).light(light).pos(drawX, drawY - 16));
        } else if (rotation == 2) {
            sortY = 0;
            options.add(texture.initDraw().sprite(0, 0, 32, 32).light(light).pos(drawX, drawY - 32));
        } else {
            sortY = 24;
            options.add(texture.initDraw().sprite(0, 1, 32, 32).light(light).pos(drawX, drawY - 16));
        }
    ...
    public void drawPreview(Level level, int tileX, int tileY, int rotation, float alpha, PlayerMob player, GameCamera camera) {
        ...
        if (rotation == 0) {
            texture.initDraw().sprite(0, 2, 32, 32).alpha(alpha).draw(drawX, drawY + 8);
        } else if (rotation == 1) {
            texture.initDraw().sprite(0, 3, 32, 32).alpha(alpha).draw(drawX, drawY - 16);
        } else if (rotation == 2) {
            texture.initDraw().sprite(0, 0, 32, 32).alpha(alpha).draw(drawX, drawY - 32);
        } else {
            texture.initDraw().sprite(0, 1, 32, 32).alpha(alpha).draw(drawX, drawY - 16);
        }
    }
    ...
    public boolean isValid(Level level, int layerID, int x, int y) {
        ...
        return this.attachesToObject(level, x, y, rotation);
    }
}
