...
extends TerrainSplatterTile {
    ...
    public void addSimulateLogic(Level level, int x, int y, long ticks, SimulatePriorityList list, boolean sendChanges) {
        GrassTile.addSimulateGrow(level, x, y, growChance, ticks, "grass", list, sendChanges);
    }
    ...
    public static void addSimulateGrow(Level level, int tileX, int tileY, double growChance, long ticks, String growObjectID, SimulatePriorityList list, boolean sendChanges) {
        GrassTile.addSimulateGrow(level, tileX, tileY, growChance, ticks, growObjectID, (object, l, x, y, r) -> object.canPlace(l, x, y, r, false) == null, list, sendChanges);
    }
    ...
    public void tick(Level level, int x, int y) {
        ...
        if (level.getObjectID(x, y) == 0 && GameRandom.globalRandom.getChance(growChance) && (grass = ObjectRegistry.getObject(ObjectRegistry.getObjectID("grass"))).canPlace(level, x, y, 0, false) == null) {
    ...
    public Point getTerrainSprite(GameTextureSection terrainTexture, Level level, int tileX, int tileY) {
        ...
        synchronized (gameRandom) {
            tile = this.drawRandom.seeded(GrassTile.getTileSeed(tileX, tileY)).nextInt(terrainTexture.getHeight() / 32);
        }
