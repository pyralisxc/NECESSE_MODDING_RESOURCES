...
public class SwampDeepCaveIncursionBiome
extends IncursionBiome {
    ...
    public LootTable getHuntDrop(IncursionData incursionData) {
        return new LootTable(new ChanceLootItem(0.66f, "silk"));
    }

    @Override
    public LootTable getBossDrop(IncursionData incursionData) {
        return new LootTable(super.getBossDrop(incursionData), LootItem.between("bioessence", 20, 25));
    }
    ...
    public IncursionLevel getNewIncursionLevel(FallenAltarObjectEntity altar, LevelIdentifier identifier, BiomeMissionIncursionData incursion, Server server, WorldEntity worldEntity, AltarData altarData) {
        return new SwampDeepCaveIncursionLevel(identifier, incursion, worldEntity, altarData);
    }

    @Override
    public ArrayList<Color> getFallenAltarGatewayColorsForBiome() {
        ArrayList<Color> gatewayColors = new ArrayList<Color>();
        gatewayColors.add(new Color(94, 140, 50));
        gatewayColors.add(new Color(133, 255, 3));
        gatewayColors.add(new Color(95, 150, 13));
        gatewayColors.add(new Color(142, 255, 3));
        gatewayColors.add(new Color(210, 244, 152));
        gatewayColors.add(new Color(247, 253, 236));
    ...
    public LootTable getExtraIncursionDrops(Mob mob) {
        ...
        if (mob.isBoss()) {
            return new LootTable(mobDrops, new LootItem("bioessence", 1));
        }
        return mobDrops;
    }
}
