...
public class SnowDeepCaveIncursionBiome
extends IncursionBiome {
    ...
    public LootTable getHuntDrop(IncursionData incursionData) {
        return new LootTable(new ChanceLootItem(0.66f, "glacialshard"));
    }

    @Override
    public LootTable getBossDrop(IncursionData incursionData) {
        return new LootTable(super.getBossDrop(incursionData), LootItem.between("cryoessence", 20, 25));
    }
    ...
    public IncursionLevel getNewIncursionLevel(FallenAltarObjectEntity altar, LevelIdentifier identifier, BiomeMissionIncursionData incursion, Server server, WorldEntity worldEntity, AltarData altarData) {
        return new SnowDeepCaveIncursionLevel(identifier, incursion, worldEntity, altarData);
    }

    @Override
    public ArrayList<Color> getFallenAltarGatewayColorsForBiome() {
        ArrayList<Color> gatewayColors = new ArrayList<Color>();
        gatewayColors.add(new Color(50, 119, 140));
        gatewayColors.add(new Color(3, 196, 255));
        gatewayColors.add(new Color(13, 139, 150));
        gatewayColors.add(new Color(3, 226, 255));
        gatewayColors.add(new Color(152, 219, 244));
        gatewayColors.add(new Color(236, 253, 253));
    ...
    public LootTable getExtraIncursionDrops(Mob mob) {
        ...
        if (mob.isBoss()) {
            return new LootTable(mobDrops, new LootItem("cryoessence", 1));
        }
        return mobDrops;
    }
}
