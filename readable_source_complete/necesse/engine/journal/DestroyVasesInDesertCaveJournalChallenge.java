...
public class DestroyVasesInDesertCaveJournalChallenge
extends ObjectsDestroyedJournalChallenge {
    public DestroyVasesInDesertCaveJournalChallenge() {
        super(30, true, "vase");
    }
    ...
    public void onObjectDestroyed(GameObject object, Level level, int layerID, int tileX, int tileY, int objectRotation, Attacker attacker, ServerClient client) {
        if (!level.isCave || !JournalChallengeUtils.isDesertBiome(level.getBiome(tileX, tileY))) {
            return;
        }
