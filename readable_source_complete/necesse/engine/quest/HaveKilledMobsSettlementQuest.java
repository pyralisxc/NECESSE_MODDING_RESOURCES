...
import necesse.engine.quest.HaveKilledMobsQuest;
...
public class HaveKilledMobsSettlementQuest
extends HaveKilledMobsQuest {
    ...
    public HaveKilledMobsSettlementQuest(GameMessage settlementName, String questTier, HaveKilledMobsQuest.HaveKilledObjective firstObjective, HaveKilledMobsQuest.HaveKilledObjective ... extraObjectives) {
        super(firstObjective, extraObjectives);
        this.settlementName = settlementName;
        this.questTier = questTier;
    }

    public HaveKilledMobsSettlementQuest(GameMessage settlementName, String questTier, String mobStringID, int mobsToKill) {
        super(mobStringID, mobsToKill);
        this.settlementName = settlementName;
        this.questTier = questTier;
    }

    public HaveKilledMobsSettlementQuest(GameMessage settlementName, String questTier, int mobID, int mobsToKill) {
    ...
    public FairType getRewardType(NetworkClient client, boolean outlined) {
        ...
        for (Item item : rewards.getItems()) {
            ...
            builder.append(ItemRegistry.getDisplayName(item.getID()));
            isFirstReward = false;
        }
        if (builder.length() == 0) {
            return null;
