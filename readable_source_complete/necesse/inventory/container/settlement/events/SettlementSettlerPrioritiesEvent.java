...
public class SettlementSettlerPrioritiesEvent
extends ContainerEvent {
    public final int settlementUniqueID;
    public final ArrayList<SettlementSettlerPrioritiesData> settlers;

    public SettlementSettlerPrioritiesEvent(ServerSettlementData data) {
        ...
        for (LevelSettler settler : data.settlers) {
            ...
            if (mob == null || !(mob instanceof EntityJobWorker)) continue;
            EntityJobWorker worker = (EntityJobWorker)((Object)mob);
            SettlementSettlerPrioritiesData priorities = new SettlementSettlerPrioritiesData(settler, worker);
            if (priorities.priorities.isEmpty()) continue;
            this.settlers.add(priorities);
        }
    }

    public SettlementSettlerPrioritiesEvent(PacketReader reader) {
        super(reader);
        ...
        for (int i = 0; i < settlersSize; ++i) {
            this.settlers.add(new SettlementSettlerPrioritiesData(reader));
        }
    }
