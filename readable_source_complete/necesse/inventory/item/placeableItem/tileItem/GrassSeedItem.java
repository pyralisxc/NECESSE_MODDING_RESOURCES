...
extends PlaceableItem {
    ...
    public GrassSeedItem(String grassStringID) {
        super(100, true);
        this.grassStringID = grassStringID;
        this.dropsAsMatDeathPenalty = true;
        this.addGlobalIngredient("anygrassseed");
    ...
    public ListGameTooltips getTooltips(InventoryItem item, PlayerMob perspective, GameBlackboard blackboard) {
        ...
        tooltips.add(Localization.translate("itemtooltip", "grassseedtip"));
        return tooltips;
    }
    ...
    public InventoryItem onPlace(Level level, int x, int y, PlayerMob player, int seed, InventoryItem item, GNDItemMap mapContent) {
        ...
        if (!event.isPrevented()) {
            GameTile t = TileRegistry.getTile(this.grassStringID);
            t.placeTile(level, tileX, tileY, true);
    ...
    public void drawPlacePreview(Level level, int x, int y, GameCamera camera, PlayerMob player, InventoryItem item, PlayerInventorySlot slot) {
        if (this.canPlace(level, x, y, player, null, item, null) == null) {
            ...
            int tileX = GameMath.getTileCoordinate(x);
            int tileY = GameMath.getTileCoordinate(y);
            TileRegistry.getTile(this.grassStringID).drawPreview(level, tileX, tileY, alpha, player, camera);
        }
    }
}
