...
public class PestWardenSpawnItem
extends ConsumableItem {
    ...
    public String canPlace(Level level, int x, int y, PlayerMob player, Line2D playerPositionLine, InventoryItem item, GNDItemMap mapContent) {
        ...
        if (!level.getIdentifier().equals(LevelIdentifier.DEEP_CAVE_IDENTIFIER) || !(level.getBiome(tileX, tileY) instanceof SwampBiome)) {
            return "notdeepswampcave";
        }
        return null;
    }
    ...
    public InventoryItem onPlace(Level level, int x, int y, PlayerMob player, int seed, InventoryItem item, GNDItemMap mapContent) {
        if (level.isServer()) {
            ...
            if (level instanceof IncursionLevel && (summonError = ((IncursionLevel)level).canSummonBoss("pestwarden")) != null) {
                if (player != null && player.isServerClient()) {
                    player.getServerClient().sendChatMessage(summonError);
                }
                return item;
            }
            System.out.println("Pest Warden has been summoned at " + level.getIdentifier() + ".");
            ...
            Mob mob = MobRegistry.getMob("pestwarden", level);
    ...
    public ListGameTooltips getTooltips(InventoryItem item, PlayerMob perspective, GameBlackboard blackboard) {
        ...
        tooltips.add(Localization.translate("itemtooltip", "decayingleaftip1"));
        tooltips.add(Localization.translate("itemtooltip", "decayingleaftip2"));
        return tooltips;
    }
