...
public class QueenSpiderSpawnItem
extends ConsumableItem {
    ...
    public String canPlace(Level level, int x, int y, PlayerMob player, Line2D playerPositionLine, InventoryItem item, GNDItemMap mapContent) {
        ...
        if (!level.getIdentifier().equals(LevelIdentifier.CAVE_IDENTIFIER) || !(level.getBiome(tileX, tileY) instanceof SnowBiome)) {
            return "notsnow";
        }
        return null;
    }
    ...
    public InventoryItem onPlace(Level level, int x, int y, PlayerMob player, int seed, InventoryItem item, GNDItemMap mapContent) {
        if (level.isServer()) {
            ...
            if (level instanceof IncursionLevel && (summonError = ((IncursionLevel)level).canSummonBoss("queenspider")) != null) {
                if (player != null && player.isServerClient()) {
                    player.getServerClient().sendChatMessage(summonError);
                }
                return item;
            }
            System.out.println("Queen Spider has been summoned at " + level.getIdentifier() + ".");
    ...
    public ListGameTooltips getTooltips(InventoryItem item, PlayerMob perspective, GameBlackboard blackboard) {
        ...
        tooltips.add(Localization.translate("itemtooltip", "royaleggtip1"));
        tooltips.add(Localization.translate("itemtooltip", "royaleggtip2"));
        tooltips.add(Localization.translate("itemtooltip", "royaleggtip3"));
        return tooltips;
    }
