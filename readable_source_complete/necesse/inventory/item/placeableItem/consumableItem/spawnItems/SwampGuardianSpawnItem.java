...
public class SwampGuardianSpawnItem
extends ConsumableItem {
    ...
    public String canPlace(Level level, int x, int y, PlayerMob player, Line2D playerPositionLine, InventoryItem item, GNDItemMap mapContent) {
        ...
        if (!(level.getBiome(tileX, tileY) instanceof SwampBiome)) {
            return "notswamp";
        }
        return null;
    }
    ...
    public InventoryItem onPlace(Level level, int x, int y, PlayerMob player, int seed, InventoryItem item, GNDItemMap mapContent) {
        if (level.isServer()) {
            ...
            if (level instanceof IncursionLevel && (summonError = ((IncursionLevel)level).canSummonBoss("swampguardian")) != null) {
                if (player != null && player.isServerClient()) {
                    player.getServerClient().sendChatMessage(summonError);
                }
                return item;
            }
            System.out.println("Swamp Guardian has been summoned at " + level.getIdentifier() + ".");
            ...
            Mob mob = MobRegistry.getMob("swampguardian", level);
    ...
    public ListGameTooltips getTooltips(InventoryItem item, PlayerMob perspective, GameBlackboard blackboard) {
        ...
        tooltips.add(Localization.translate("itemtooltip", "spikedfossiltip1"));
        tooltips.add(Localization.translate("itemtooltip", "spikedfossiltip2"));
        return tooltips;
    }
