...
public class AncientVultureSpawnItem
extends ConsumableItem {
    ...
    public String canPlace(Level level, int x, int y, PlayerMob player, Line2D playerPositionLine, InventoryItem item, GNDItemMap mapContent) {
        ...
        if (!(level.getBiome(tileX, tileY) instanceof DesertBiome)) {
            return "notdesert";
        }
        return null;
    }
    ...
    public InventoryItem onPlace(Level level, int x, int y, PlayerMob player, int seed, InventoryItem item, GNDItemMap mapContent) {
        if (level.isServer()) {
            ...
            if (level instanceof IncursionLevel && (summonError = ((IncursionLevel)level).canSummonBoss("ancientvulture")) != null) {
                if (player != null && player.isServerClient()) {
                    player.getServerClient().sendChatMessage(summonError);
                }
                return item;
            }
            System.out.println("Ancient Vulture has been summoned at " + level.getIdentifier() + ".");
    ...
    public ListGameTooltips getTooltips(InventoryItem item, PlayerMob perspective, GameBlackboard blackboard) {
        ...
        tooltips.add(Localization.translate("itemtooltip", "ancientstatuetip1"));
        tooltips.add(Localization.translate("itemtooltip", "ancientstatuetip2"));
        return tooltips;
    }
