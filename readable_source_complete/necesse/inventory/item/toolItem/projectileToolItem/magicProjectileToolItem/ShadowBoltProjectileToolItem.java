...
public class ShadowBoltProjectileToolItem
extends MagicProjectileToolItem {
    public ShadowBoltProjectileToolItem() {
        ...
        this.attackDamage.setBaseValue(84.0f).setUpgradedValue(1.0f, 154.00006f);
    ...
    public ListGameTooltips getPreEnchantmentTooltips(InventoryItem item, PlayerMob perspective, GameBlackboard blackboard) {
        ...
        tooltips.add(Localization.translate("itemtooltip", "shadowbolttip"));
        return tooltips;
    }

    @Override
    public InventoryItem onAttack(Level level, int x, int y, ItemAttackerMob attackerMob, int attackHeight, InventoryItem item, ItemAttackSlot slot, int animAttack, int seed, GNDItemMap mapContent) {
        ShadowBoltProjectile projectile = new ShadowBoltProjectile(level, attackerMob, attackerMob.x, attackerMob.y, x, y, this.getProjectileVelocity(item, attackerMob), this.getAttackRange(item), this.getAttackDamage(item), this.getKnockback(item, attackerMob));
    ...
    protected SoundSettings getAttackSound() {
        return new SoundSettings(GameResources.shadowBolt).volume(0.2f);
    }
}
