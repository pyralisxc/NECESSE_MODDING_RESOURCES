...
public class HeavyHammerProjectileToolItem
extends MeleeProjectileToolItem {
    public HeavyHammerProjectileToolItem() {
        ...
        this.attackDamage.setBaseValue(18.0f).setUpgradedValue(1.0f, 81.66669f);
    ...
    protected SoundSettings getAttackSound() {
        return new SoundSettings(GameResources.heavyHammer).volume(0.1f);
    }

    @Override
    public ListGameTooltips getPreEnchantmentTooltips(InventoryItem item, PlayerMob perspective, GameBlackboard blackboard) {
        ListGameTooltips tooltips = super.getPreEnchantmentTooltips(item, perspective, blackboard);
        tooltips.add(Localization.translate("itemtooltip", "heavyhammertip"));
        return tooltips;
    }

    @Override
    public InventoryItem onAttack(Level level, int x, int y, ItemAttackerMob attackerMob, int attackHeight, InventoryItem item, ItemAttackSlot slot, int animAttack, int seed, GNDItemMap mapContent) {
        Projectile projectile = ProjectileRegistry.getProjectile("stone", level, attackerMob.x, attackerMob.y, (float)x, (float)y, (float)this.getProjectileVelocity(item, attackerMob), this.getAttackRange(item), this.getAttackDamage(item), this.getKnockback(item, attackerMob), (Mob)attackerMob);
