...
public class SparklerProjectile
extends Projectile {
    ...
    public SparklerProjectile(Level level, float x, float y, float targetX, float targetY, int distance, GameDamage damage, Mob owner) {
        this.setLevel(level);
        this.x = x;
        this.y = y;
    ...
    public void spawnSprayParticles(int amount) {
        ...
        for (int i = 0; i < amount; ++i) {
            ...
            this.getLevel().entityManager.addParticle(posX, posY, particleTypeSwitcher.next()).fadesAlphaTime(0, timeToFadeOut).color(new Color(255, 130, 28)).sizeFadesInAndOut(15, 20, 100, 0).height(heightMover).rotates().moves((pos, delta, lifeTime, timeAlive, lifePercent) -> {
