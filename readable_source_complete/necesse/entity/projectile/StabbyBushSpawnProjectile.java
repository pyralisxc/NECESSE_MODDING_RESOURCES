...
public class StabbyBushSpawnProjectile
extends Projectile {
    private SummonToolItem summonToolItem;
    private InventoryItem inventoryItem;

    public StabbyBushSpawnProjectile() {
    }

    public StabbyBushSpawnProjectile(Level level, Mob owner, float x, float y, float targetX, float targetY, float speed, int distance, GameDamage damage, int knockback, SummonToolItem summonToolItem, InventoryItem inventoryItem) {
        this.setLevel(level);
        this.x = x;
        this.y = y;
        this.speed = speed;
    ...
    public StabbyBushSpawnProjectile(Level level, Mob owner, float targetX, float targetY, float speed, int distance, GameDamage damage, int knockback, SummonToolItem summonToolItem, InventoryItem inventoryItem) {
    ...
    public void doHitLogic(Mob mob, LevelObjectHit object, float x, float y) {
        ...
        if (owner != null && !owner.removed() && owner.isItemAttacker) {
            StabbyBushFollowingMob stabbyBushMob = new StabbyBushFollowingMob();
            ((ItemAttackerMob)owner).serverFollowersManager.addFollower("stabbybushfollowingmob", (Mob)stabbyBushMob, FollowPosition.WALK_CLOSE, "summonedmob", 1.0f, this.summonToolItem.getMaxSummons(this.inventoryItem, (ItemAttackerMob)owner), null, false);
            Point2D.Float spawnPoint = new Point2D.Float(x, y);
            stabbyBushMob.updateDamage(this.summonToolItem.getAttackDamage(this.inventoryItem));
            stabbyBushMob.setEnchantment(this.summonToolItem.getEnchantment(this.inventoryItem));
            if (!owner.isPlayer) {
                stabbyBushMob.setRemoveWhenNotInInventory(ItemRegistry.getItem("stabbybush"), CheckSlotType.WEAPON);
            }
            owner.getLevel().entityManager.addMob(stabbyBushMob, spawnPoint.x, spawnPoint.y);
        }
    }
    ...
    public void addDrawables(List<LevelSortedDrawable> list, OrderableDrawables tileList, OrderableDrawables topList, OrderableDrawables overlayList, Level level, TickManager tickManager, GameCamera camera, PlayerMob perspective) {
        ...
        HumanDrawOptions humanDrawOptions = new HumanDrawOptions(level, MobRegistry.Textures.stabbyBush).sprite(0, 2, 64).light(light).rotate(angle, 32, 32);
