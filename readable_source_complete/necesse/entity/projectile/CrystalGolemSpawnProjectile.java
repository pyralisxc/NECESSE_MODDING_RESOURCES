...
public class CrystalGolemSpawnProjectile
extends Projectile {
    public CrystalGolemSpawnProjectile() {
    }

    public CrystalGolemSpawnProjectile(Level level, Mob owner, float x, float y, float targetX, float targetY, float speed, int distance, GameDamage damage, int knockback) {
        this.setLevel(level);
        this.x = x;
        this.y = y;
        this.speed = speed;
    ...
    public CrystalGolemSpawnProjectile(Level level, Mob owner, float targetX, float targetY, float speed, int distance, GameDamage damage, int knockback) {
    ...
    public void doHitLogic(Mob mob, LevelObjectHit object, float x, float y) {
        ...
        if (owner != null && !owner.removed()) {
            Mob crystalGolem = MobRegistry.getMob("crystalgolem", this.getLevel());
            crystalGolem.isSummoned = true;
            if (!crystalGolem.collidesWith(this.getLevel(), (int)x, (int)y)) {
                this.getLevel().entityManager.addMob(crystalGolem, (int)x, (int)y);
            }
        }
    }
    ...
    public void addDrawables(List<LevelSortedDrawable> list, OrderableDrawables tileList, OrderableDrawables topList, OrderableDrawables overlayList, Level level, TickManager tickManager, GameCamera camera, PlayerMob perspective) {
        ...
        TextureDrawOptionsEnd options = MobRegistry.Textures.crystalGolem.initDraw().sprite(0, 2, 64).light(light).rotate(angle, 32, 32).pos(drawX, drawY - (int)this.getHeight());
