...
public class SwampDwellerStaffPetalProjectile
extends Projectile {
    public SwampDwellerStaffPetalProjectile() {
    }

    public SwampDwellerStaffPetalProjectile(Level level, Mob owner, float x, float y, float targetX, float targetY, float speed, int distance, GameDamage damage, int knockback) {
        this.setLevel(level);
        this.setOwner(owner);
        this.x = x;
        this.y = y;
    ...
    protected SoundSettings getSpawnSound() {
        return new SoundSettings(GameResources.magicSwing).volume(0.3f);
    }
}
