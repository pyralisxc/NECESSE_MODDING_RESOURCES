...
public class SprinklerProjectile
extends Projectile {
    private boolean hitMob = false;

    public SprinklerProjectile() {
    }

    public SprinklerProjectile(Level level, float x, float y, float targetX, float targetY, int distance, GameDamage damage, Mob owner) {
        this.setLevel(level);
        this.x = x;
        this.y = y;
    ...
    public void spawnSprayParticles(int amount) {
        ...
        for (int i = 0; i < amount; ++i) {
            ...
            this.getLevel().entityManager.addParticle(posX, posY, particleTypeSwitcher.next()).fadesAlphaTime(0, timeToFadeOut).color(new Color(57, 180, 240)).sizeFadesInAndOut(15, 20, 100, 0).height(heightMover).rotates().moves((pos, delta, lifeTime, timeAlive, lifePercent) -> {
                if (!(!(heightMover.currentHeight > endHeight) || this.removed() && this.hitMob)) {
                    mover.tick(pos, delta, lifeTime, timeAlive, lifePercent);
                }
            }).sprite(GameResources.bubbleParticle.sprite(0, 0, 12)).lifeTime(750);
        }
    }
    ...
    public void doHitLogic(Mob mob, LevelObjectHit object, float x, float y) {
        ...
        boolean bl = this.hitMob = mob != null;
        if (this.isServer() && mob != null) {
            ActiveBuff ab = new ActiveBuff(BuffRegistry.Debuffs.FROSTSLOW, mob, 5.0f, (Attacker)this.getOwner());
            mob.addBuff(ab, true);
        }
    }
