...
public class BoulderStaffProjectile
extends Projectile {
    ...
    public BoulderStaffProjectile(Level level, Mob owner, float x, float y, float targetX, float targetY, float speed, int distance, GameDamage damage, int knockback) {
        this.setLevel(level);
        this.x = x;
        this.y = y;
        this.speed = speed;
    ...
    public void setupPositionPacket(PacketWriter writer) {
        ...
        writer.putNextFloat(this.boulderHeight);
    }
    ...
    public void applyPositionPacket(PacketReader reader) {
        ...
        this.boulderHeight = reader.getNextFloat();
    }
    ...
    public float tickMovement(float delta) {
        ...
        this.boulderHeight += this.deltaHeight * delta / 250.0f;
        if (this.boulderHeight < 0.0f) {
            if (this.isClient()) {
                SoundManager.playSound(GameResources.punch, (SoundEffect)SoundEffect.effect(this).volume(0.5f).pitch(1.0f));
                ExplosionEvent.spawnExplosionParticles(this.getLevel(), this.x, this.y, 20, 0.0f, 20.0f, (level, x, y, dirX, dirY, lifeTime, range) -> level.entityManager.addParticle(x, y, Particle.GType.CRITICAL).movesConstant(dirX, dirY).color(new Color(50, 50, 50)).lifeTime(lifeTime));
                SoundManager.playSound(GameResources.blunthit, (SoundEffect)SoundEffect.effect(this).volume(0.45f).falloffDistance(1000));
            }
    ...
    public Trail getTrail() {
        Trail trail = new Trail(this, this.getLevel(), new Color(50, 50, 50), 0.0f, 500, this.getHeight());
