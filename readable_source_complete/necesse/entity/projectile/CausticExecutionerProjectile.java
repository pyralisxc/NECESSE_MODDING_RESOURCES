...
public class CausticExecutionerProjectile
extends Projectile {
    ...
    public CausticExecutionerProjectile(Level level, float x, float y, float targetX, float targetY, int distance, GameDamage damage, Mob owner) {
        this.setLevel(level);
        this.x = x;
        this.y = y;
    ...
    public void spawnSprayParticles(int amount) {
        ...
        for (int i = 0; i < amount; ++i) {
            ...
            this.getLevel().entityManager.addParticle(posX, posY, particleTypeSwitcher.next()).fadesAlphaTime(0, timeToFadeOut).color(new Color(166, 204, 52)).sizeFadesInAndOut(6, 12, 100, 0).sprite(GameResources.puffParticles.sprite(GameRandom.globalRandom.nextInt(4), 0, 12)).height(heightMover).rotates().moves((pos, delta, lifeTime, timeAlive, lifePercent) -> {
