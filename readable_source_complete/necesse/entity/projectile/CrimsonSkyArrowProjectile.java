...
public class CrimsonSkyArrowProjectile
extends Projectile {
    ...
    public CrimsonSkyArrowProjectile(Level level, Mob owner, float x, float y, float targetX, float targetY, float speed, int distance, GameDamage damage, float eventResilienceGain, int knockback, Point2D.Float targetPoints, boolean isFallingProjectile) {
        this.setLevel(level);
        this.setOwner(owner);
        this.x = x;
        this.y = y;
    ...
    public void doHitLogic(Mob mob, LevelObjectHit object, float x, float y) {
        ...
        if (!this.isFallingProjectile) {
            if (this.targetPoints != null) {
                CrimsonSkyArrowProjectile projectile = new CrimsonSkyArrowProjectile(this.getLevel(), this.getOwner(), this.targetPoints.x, this.targetPoints.y - (float)this.distance, this.targetPoints.x, this.targetPoints.y, this.speed, this.distance, this.damage, this.eventResilienceGain, this.knockback, null, true);
                projectile.getUniqueID(new GameRandom(this.getUniqueID()).nextSeeded(67));
            ...
            if (!this.isServer()) {
                ...
                explosion.colorGetter = (particle, progress, random) -> ParticleOption.randomizeColor(348.0f, 0.94f, 0.8f, 2.0f, 0.0f, 0.0f);
            ...
            if (!this.isClient()) {
                ...
                this.streamTargets(this.getOwner(), targetBox).filter(m -> m.canBeHit(this) && m.getDistance(targetX, targetY) <= (float)range).forEach(m -> m.isServerHit(this.getDamage(), m.x - x, m.y - y, this.knockback, this));
            }
            this.getLevel().entityManager.events.addHidden(new TheCrimsonSkyEvent(this.getOwner(), (int)x, (int)y, new GameRandom(this.getUniqueID()), this.getDamage(), this.eventResilienceGain));
        }
    }
