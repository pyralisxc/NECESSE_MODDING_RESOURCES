...
public class AscendedGolemSpawnProjectile
extends Projectile {
    protected long spawnTime;

    public AscendedGolemSpawnProjectile() {
    }

    public AscendedGolemSpawnProjectile(Level level, Mob owner, float x, float y, float targetX, float targetY, float speed, int distance, GameDamage damage, int knockback) {
        this.setLevel(level);
        this.x = x;
        this.y = y;
        this.speed = speed;
    ...
    public AscendedGolemSpawnProjectile(Level level, Mob owner, float targetX, float targetY, float speed, int distance, GameDamage damage, int knockback) {
    ...
    public void doHitLogic(Mob mob, LevelObjectHit object, float x, float y) {
        ...
        if (owner != null && !owner.removed()) {
            Mob golemMob = MobRegistry.getMob("ascendedgolem", this.getLevel());
            golemMob.isSummoned = true;
            if (!golemMob.collidesWith(this.getLevel(), (int)x, (int)y)) {
                this.getLevel().entityManager.addMob(golemMob, (int)x, (int)y);
                if (owner instanceof AscendedWizardMob) {
                    ((AscendedWizardMob)owner).spawnedMobs.add(golemMob);
                }
            }
        }
    }
    ...
    protected void spawnDeathParticles() {
        ...
        SoundManager.playSound(GameResources.crystalHit2, (SoundEffect)SoundEffect.effect(this.x, this.y).volume(0.7f).pitch(pitch.floatValue()).falloffDistance(1400));
    }
    ...
    public void addDrawables(List<LevelSortedDrawable> list, OrderableDrawables tileList, OrderableDrawables topList, OrderableDrawables overlayList, Level level, TickManager tickManager, GameCamera camera, PlayerMob perspective) {
        ...
        TextureDrawOptionsEnd options = MobRegistry.Textures.ascendedGolem.initDraw().sprite(0, 2, 64).light(light).rotate(angle, 32, 32).pos(drawX, drawY - (int)this.getHeight());
    ...
    protected SoundSettings getSpawnSound() {
        return new SoundSettings(GameResources.spit).volume(0.9f).fallOffDistance(1200);
    }
}
