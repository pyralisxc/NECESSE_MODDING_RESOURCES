...
public class AscendedSlimeQuakeWarningEvent
extends WeaponShockWaveLevelEvent {
    ...
    public AscendedSlimeQuakeWarningEvent() {
        super(360.0f, 135.0f, 50.0f);
    }

    public AscendedSlimeQuakeWarningEvent(Mob owner, int x, int y, GameRandom uniqueIDRandom, float targetAngle, float angleExtent, float velocity, float range, int warningTime, float offset) {
        super(owner, x, y, uniqueIDRandom, targetAngle, angleExtent, 135.0f, 50.0f, new GameDamage(0.0f), 0.0f, velocity, 0.0f, range);
        ...
        if (this.isClient()) {
            this.level.entityManager.addPillarHandler(new GroundPillarHandler<GroundPillar>(this.pillars){
                ...
                protected boolean canRemove() {
                    return AscendedSlimeQuakeWarningEvent.this.isOver();
                }
        ...
        if (this.isClient()) {
            ...
            synchronized (groundPillarList) {
                for (Point2D.Float pos : this.getPositionsAlongHit(radius, startAngle, endAngle, 20.0f, false)) {
                    this.pillars.add(new AscendedSlimePillar((int)(pos.x + GameRandom.globalRandom.getFloatBetween(-10.0f, 10.0f)), (int)(pos.y + GameRandom.globalRandom.getFloatBetween(-10.0f, 10.0f)), radius, this.level.getWorldEntity().getLocalTime(), this.warningTime));
                }
    ...
    extends GroundPillar {
        ...
        public AscendedSlimePillar(int x, int y, double spawnDistance, long spawnTime, int warningTime) {
            super(x, y, spawnDistance, spawnTime);
            GameTexture pillarSprites = GameResources.ascendedSlimeGround;
            if (pillarSprites != null) {
                ...
                this.texture = new GameTextureSection(GameResources.ascendedSlimeGround).sprite(sprite, 0, res);
            }
