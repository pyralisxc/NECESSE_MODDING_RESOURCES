...
extends WeaponShockWaveLevelEvent {
    ...
    public SlimeQuakeEvent(Mob owner, int x, int y, GameRandom uniqueIDRandom, float targetAngle, GameDamage damage, float velocity, float knockback, float range, float offset) {
        super(owner, x, y, uniqueIDRandom, targetAngle, 360.0f, 135.0f, 50.0f, damage, 0.0f, velocity, knockback, range);
        ...
        if (this.isClient()) {
            ...
            synchronized (groundPillarList) {
                for (Point2D.Float pos : this.getPositionsAlongHit(radius, startAngle, endAngle, 20.0f, false)) {
                    this.pillars.add(new SlimePillar((int)(pos.x + GameRandom.globalRandom.getFloatBetween(-10.0f, 10.0f)), (int)(pos.y + GameRandom.globalRandom.getFloatBetween(-10.0f, 10.0f)), radius, this.level.getWorldEntity().getLocalTime()));
                }
    ...
    extends GroundPillar {
        ...
        public SlimePillar(int x, int y, double spawnDistance, long spawnTime) {
            super(x, y, spawnDistance, spawnTime);
            GameTexture pillarSprites = GameResources.slimeSpike;
            if (pillarSprites != null) {
                ...
                this.texture = new GameTextureSection(GameResources.slimeSpike).sprite(sprite, 0, res);
            }
