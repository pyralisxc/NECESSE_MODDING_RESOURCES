...
extends MountFollowingMob {
    ...
    protected SoundSettings getAmbientSound() {
        if (this.inLiquidFloat() > 0.0f) {
            return new SoundSettings(GameResources.steelBoat).volume(0.3f);
        }
    ...
    public void addDrawables(List<MobDrawable> list, OrderableDrawables tileList, OrderableDrawables topList, Level level, int x, int y, TickManager tickManager, GameCamera camera, PlayerMob perspective) {
        ...
        GameLight light = level.getLightLevel(SteelBoatMob.getTileCoordinate(x), SteelBoatMob.getTileCoordinate(y));
        int drawX = camera.getDrawX(x) - 32;
        int drawY = camera.getDrawY(y) - 47 + 5;
        int dir = this.getDir();
        Point sprite = this.getAnimSprite(x, y, dir);
        drawY += this.getBobbing(x, y);
        final TextureDrawOptionsEnd behind = MobRegistry.Textures.steelBoat.initDraw().sprite(sprite.x, sprite.y, 64).light(light).pos(drawX, drawY += level.getTile(SteelBoatMob.getTileCoordinate(x), SteelBoatMob.getTileCoordinate(y)).getMobSinkingAmount(this));
