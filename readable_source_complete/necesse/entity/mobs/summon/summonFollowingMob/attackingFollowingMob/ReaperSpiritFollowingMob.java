...
public class ReaperSpiritFollowingMob
extends FlyingAttackingFollowingMob {
    ...
    public ReaperSpiritFollowingMob() {
        ...
        this.ambientSoundCooldownMin = 3000;
    ...
    public void init() {
        super.init();
        this.ai = new BehaviourTreeAI<ReaperSpiritFollowingMob>(this, new PlayerCirclingFollowerCollisionChaserAI(384, null, 15, -1, 50), new FlyingAIMover());
        if (this.isClient()) {
    ...
    protected SoundSettings getDeathSound() {
        return new SoundSettings(GameResources.reaperSpiritDeath).volume(0.2f);
    }

    @Override
    protected void addDrawables(List<MobDrawable> list, OrderableDrawables tileList, OrderableDrawables topList, Level level, int x, int y, TickManager tickManager, GameCamera camera, PlayerMob perspective) {
        super.addDrawables(list, tileList, topList, level, x, y, tickManager, camera, perspective);
        GameLight light = level.getLightLevel(ReaperSpiritFollowingMob.getTileCoordinate(x), ReaperSpiritFollowingMob.getTileCoordinate(y));
        int drawX = camera.getDrawX(x) - 16;
        int drawY = camera.getDrawY(y) - 16;
        TextureDrawOptionsEnd body = MobRegistry.Textures.reaperSpiritPet.initDraw().sprite(0, 0, 32).light(light).rotate(this.moveAngle, 16, 16).pos(drawX, drawY);
        int minLight = 100;
        TextureDrawOptionsEnd eyes = MobRegistry.Textures.reaperSpiritPet.initDraw().sprite(1, 0, 32).light(light.minLevelCopy(minLight)).rotate(this.moveAngle, 16, 16).pos(drawX, drawY);
        topList.add(tm -> {
            body.draw();
            eyes.draw();
        });
