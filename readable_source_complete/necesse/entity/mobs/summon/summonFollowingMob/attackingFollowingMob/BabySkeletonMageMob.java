...
public class BabySkeletonMageMob
extends AttackingFollowingMob {
    ...
    public void init() {
        super.init();
        this.ai = new BehaviourTreeAI<BabySkeletonMageMob>(this, new PlayerFollowerChaserAI<BabySkeletonMageMob>(576, 320, false, false, 640, 64){

            @Override
            public boolean attackTarget(BabySkeletonMageMob mob, Mob target) {
                if (mob.canAttack()) {
                    mob.attack(target.getX(), target.getY(), false);
                    Projectile projectile = ProjectileRegistry.getProjectile("babybone", mob.getLevel(), mob.x, mob.y, target.x, target.y, 150.0f, 640, BabySkeletonMageMob.this.summonDamage, (Mob)mob);
    ...
    public void addDrawables(List<MobDrawable> list, OrderableDrawables tileList, OrderableDrawables topList, Level level, int x, int y, TickManager tickManager, GameCamera camera, PlayerMob perspective) {
        ...
        GameLight light = level.getLightLevel(BabySkeletonMageMob.getTileCoordinate(x), BabySkeletonMageMob.getTileCoordinate(y));
        int drawX = camera.getDrawX(x) - 32;
        int drawY = camera.getDrawY(y) - 44 - 7;
        int dir = this.getDir();
        Point sprite = this.getAnimSprite(x, y, dir);
        drawY += this.getBobbing(x, y);
        drawY += level.getTile(BabySkeletonMageMob.getTileCoordinate(x), BabySkeletonMageMob.getTileCoordinate(y)).getMobSinkingAmount(this);
