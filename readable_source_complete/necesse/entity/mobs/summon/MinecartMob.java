...
public class MinecartMob
extends SummonedMob {
    public static LootTable lootTable = new LootTable(new LootItem("minecart"));
                ...
                if (object instanceof MinecartTrackObject && !(object instanceof TrapTrackObject)) {
                    ...
                    if (pos != null) {
                        ...
                        if (this.minecartSpeed > 0.0f) {
                            ...
                            int dir = this.getDir();
                            float f = dir == 1 || dir == 3 ? this.minecartSpeed * (float)(dir == 1 ? 1 : -1) : (this.dx = 0.0f);
                            float f2 = dir == 0 || dir == 2 ? this.minecartSpeed * (float)(dir == 2 ? 1 : -1) : (this.dy = 0.0f);
    ...
    public boolean canBeHit(Attacker attacker) {
        ...
        if (attacker instanceof ToolItemMobAbilityEvent || attacker instanceof ExplosionEvent) {
            return attacker.getAttackOwners().stream().anyMatch(m -> m.isPlayer);
        }
        return false;
    }
    ...
    public void addDrawables(List<MobDrawable> list, OrderableDrawables tileList, OrderableDrawables topList, Level level, int x, int y, TickManager tickManager, GameCamera camera, PlayerMob perspective) {
        ...
        GameLight light = level.getLightLevel(MinecartMob.getTileCoordinate(x), MinecartMob.getTileCoordinate(y));
        int drawX = camera.getDrawX(x) - 32;
        int drawY = camera.getDrawY(y) - 47;
        Point sprite = this.getAnimSprite(x, y, this.minecartDir);
        drawY += this.getBobbing(x, y);
        final MaskShaderOptions swimMask = this.getSwimMaskShaderOptions(this.inLiquidFloat(x, y));
        final TextureDrawOptionsEnd behind = MobRegistry.Textures.minecart.initDraw().sprite(sprite.x, sprite.y, 64).addMaskShader(swimMask).light(light).pos(drawX, drawY += level.getTile(MinecartMob.getTileCoordinate(x), MinecartMob.getTileCoordinate(y)).getMobSinkingAmount(this));
    ...
    public static void drawPlacePreview(Level level, int levelX, int levelY, int dir, GameCamera camera) {
        ...
        if (mob != null) {
            ...
            if (object instanceof MinecartTrackObject) {
                ...
                if (pos != null) {
                    ...
                    MobRegistry.Textures.minecart.initDraw().sprite(sprite.x, sprite.y, 64).alpha(0.5f).draw(drawX, drawY += level.getTile(MinecartMob.getTileCoordinate(pos.x), MinecartMob.getTileCoordinate(pos.y)).getMobSinkingAmount(mob));
                    return;
                }
            }
        }
        ...
        MobRegistry.Textures.minecart.initDraw().sprite(0, dir, 64).alpha(0.5f).draw(drawX, drawY += level.getLevelTile(MinecartMob.getTileCoordinate(levelX), MinecartMob.getTileCoordinate(levelY)).getLiquidBobbing());
    }
    ...
    protected TextureDrawOptions getShadowDrawOptions(Level level, int x, int y, GameLight light, GameCamera camera) {
        return MinecartMob.getShadowDrawOptions(this, level, x, y, 0, this.minecartDir, light, camera);
    }
    ...
    public static TextureDrawOptions getShadowDrawOptions(Mob mob, Level level, int x, int y, int yOffset, int dir, GameLight light, GameCamera camera) {
        ...
        return shadowTexture.initDraw().sprite(0, dir, 64).light(light).pos(drawX, drawY += level.getTile(MinecartMob.getTileCoordinate(x), MinecartMob.getTileCoordinate(y)).getMobSinkingAmount(mob));
    }
