...
public class WindBootsActiveBuff
extends Buff
implements MovementTickBuff {
    public WindBootsActiveBuff() {
    ...
    public void init(ActiveBuff buff, BuffEventSubscriber eventSubscriber) {
        ...
        int strength = 150;
        Point2D.Float dir = PacketForceOfWind.getMobDir(owner);
        PacketForceOfWind.applyToMob(level, owner, dir.x, dir.y, strength);
        PacketForceOfWind.addCooldownStack(owner, 3.0f, false);
        owner.buffManager.addBuff(new ActiveBuff(BuffRegistry.FOW_ACTIVE, owner, 0.15f, null), false);
    ...
    public void tickMovement(ActiveBuff buff, float delta) {
        ...
        if (!buff.owner.inLiquid() && (owner = buff.owner).isClient() && (owner.dx != 0.0f || owner.dy != 0.0f)) {
            ...
            if (particleBuffer >= 15.0f) {
                ...
                owner.getLevel().entityManager.addParticle(owner.x + xOffset, owner.y + yOffset - 2.0f, Particle.GType.IMPORTANT_COSMETIC).color(new Color(187, 211, 213)).sizeFadesInAndOut(10, 16, 50, 200).movesConstant(owner.dx / 10.0f, owner.dy / 10.0f).lifeTime(300).height(2.0f);
            }
