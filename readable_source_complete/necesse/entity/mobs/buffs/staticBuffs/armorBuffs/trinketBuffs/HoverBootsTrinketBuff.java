...
public class HoverBootsTrinketBuff
extends TrinketBuff
implements ActiveBuffAbility {
    ...
    public void onActiveAbilityStarted(PlayerMob player, ActiveBuff buff, Packet content) {
        ...
        player.buffManager.addBuff(new ActiveBuff(BuffRegistry.HOVER_BOOTS_ACTIVE, (Mob)player, 1.0f, null), false);
    }
    ...
    public boolean tickActiveAbility(PlayerMob player, ActiveBuff buff, boolean isRunningClient) {
        long msToDeplete;
        float usage;
        ActiveBuff speedBuff = player.buffManager.getBuff(BuffRegistry.HOVER_BOOTS_ACTIVE);
        if (speedBuff != null) {
            speedBuff.setDurationLeftSeconds(1.0f);
        } else {
            player.buffManager.addBuff(new ActiveBuff(BuffRegistry.HOVER_BOOTS_ACTIVE, (Mob)player, 1.0f, null), false);
        }
        if (!(player.moveX == 0.0f && player.moveY == 0.0f || player.dx == 0.0f && player.dy == 0.0f || StaminaBuff.useStaminaAndGetValid(player, usage = 50.0f / (float)(msToDeplete = 10000L)))) {
            return false;
        }
    ...
    public void onActiveAbilityStopped(PlayerMob player, ActiveBuff buff) {
        player.buffManager.removeBuff(BuffRegistry.HOVER_BOOTS_ACTIVE, false);
    }
}
