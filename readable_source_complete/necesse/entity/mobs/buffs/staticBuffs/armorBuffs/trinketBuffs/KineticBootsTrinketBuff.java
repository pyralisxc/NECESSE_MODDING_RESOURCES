...
public class KineticBootsTrinketBuff
extends TrinketBuff
implements ActiveBuffAbility {
    ...
    public void onActiveAbilityStarted(PlayerMob player, ActiveBuff buff, Packet content) {
        ...
        if (!buff.owner.inLiquid()) {
            player.buffManager.addBuff(new ActiveBuff(BuffRegistry.KINETIC_BOOTS_ACTIVE, (Mob)player, 1.0f, null), false);
        }
    }
    ...
    public boolean tickActiveAbility(PlayerMob player, ActiveBuff buff, boolean isRunningClient) {
        if (player.inLiquid()) {
            player.buffManager.removeBuff(BuffRegistry.KINETIC_BOOTS_ACTIVE, false);
        } else {
            long msToDeplete;
            float usage;
            ActiveBuff speedBuff = player.buffManager.getBuff(BuffRegistry.KINETIC_BOOTS_ACTIVE);
            if (speedBuff != null) {
