...
public class LeatherDashersTrinketBuff
extends TrinketBuff
implements ActiveBuffAbility {
    ...
    public void onActiveAbilityStarted(PlayerMob player, ActiveBuff buff, Packet content) {
        ...
        if (!buff.owner.inLiquid()) {
            player.buffManager.addBuff(new ActiveBuff(BuffRegistry.LEATHER_DASHERS_ACTIVE, (Mob)player, 1.0f, null), false);
        }
    }
    ...
    public boolean tickActiveAbility(PlayerMob player, ActiveBuff buff, boolean isRunningClient) {
        if (player.inLiquid()) {
            player.buffManager.removeBuff(BuffRegistry.LEATHER_DASHERS_ACTIVE, false);
        } else {
            long msToDeplete;
            float usage;
            ActiveBuff speedBuff = player.buffManager.getBuff(BuffRegistry.LEATHER_DASHERS_ACTIVE);
            if (speedBuff != null) {
                speedBuff.setDurationLeftSeconds(1.0f);
            } else {
                player.buffManager.addBuff(new ActiveBuff(BuffRegistry.LEATHER_DASHERS_ACTIVE, (Mob)player, 1.0f, null), false);
            }
            if (!(player.moveX == 0.0f && player.moveY == 0.0f || player.dx == 0.0f && player.dy == 0.0f || StaminaBuff.useStaminaAndGetValid(player, usage = 50.0f / (float)(msToDeplete = 3000L)))) {
                return false;
            }
        }
