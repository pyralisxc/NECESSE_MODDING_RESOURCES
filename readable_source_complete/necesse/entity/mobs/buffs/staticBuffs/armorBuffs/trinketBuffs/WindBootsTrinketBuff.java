...
public class WindBootsTrinketBuff
extends TrinketBuff
implements ActiveBuffAbility {
    ...
    public void onActiveAbilityStarted(PlayerMob player, ActiveBuff buff, Packet content) {
        ...
        player.buffManager.addBuff(new ActiveBuff(BuffRegistry.WIND_BOOTS_ACTIVE, (Mob)player, 1.0f, null), false);
    }

    @Override
    public boolean tickActiveAbility(PlayerMob player, ActiveBuff buff, boolean isRunningClient) {
        if (player.inLiquid()) {
            player.buffManager.removeBuff(BuffRegistry.WIND_BOOTS_ACTIVE, false);
        } else {
            long msToDeplete;
            float usage;
            ActiveBuff speedBuff = player.buffManager.getBuff(BuffRegistry.WIND_BOOTS_ACTIVE);
            if (speedBuff != null) {
