...
public class SpiderSetBonusBuff
extends SetBonusBuff {
    ...
    public void onHasAttacked(ActiveBuff buff, MobWasHitEvent event) {
        ...
        if (!event.wasPrevented) {
            event.target.buffManager.addBuff(new ActiveBuff(BuffRegistry.Debuffs.SPIDER_VENOM, event.target, 5.0f, event.attacker), event.target.isServer());
        }
    }
