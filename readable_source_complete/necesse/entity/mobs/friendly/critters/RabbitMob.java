...
extends CritterMob {
    ...
    public void spawnDeathParticles(float knockbackX, float knockbackY) {
        for (int i = 0; i < 4; ++i) {
            this.getLevel().entityManager.addParticle(new FleshParticle(this.getLevel(), this.getTexture(), i, 8, 32, this.x, this.y, 20.0f, knockbackX, knockbackY), Particle.GType.IMPORTANT_COSMETIC);
        }
    }

    protected GameTexture getTexture() {
        return MobRegistry.Textures.rabbit;
    }

    @Override
    public void addDrawables(List<MobDrawable> list, OrderableDrawables tileList, OrderableDrawables topList, Level level, int x, int y, TickManager tickManager, GameCamera camera, PlayerMob perspective) {
        super.addDrawables(list, tileList, topList, level, x, y, tickManager, camera, perspective);
        GameLight light = level.getLightLevel(RabbitMob.getTileCoordinate(x), RabbitMob.getTileCoordinate(y));
        ...
        final TextureDrawOptionsEnd options = this.getTexture().initDraw().sprite(sprite.x, sprite.y, 64).addMaskShader(swimMask).mirror(mirror, false).light(light).pos(drawX, drawY += level.getTile(RabbitMob.getTileCoordinate(x), RabbitMob.getTileCoordinate(y)).getMobSinkingAmount(this));
    ...
    public MobSpawnLocation checkSpawnLocation(MobSpawnLocation location) {
        return super.checkSpawnLocation(location).checkTile((tileX, tileY) -> {
            ...
            return this.canSpawnOnTileID(tileID);
        });
    }
    ...
    protected SoundSettings getAmbientSound() {
        return new SoundSettings(GameRandom.globalRandom.getOneOf(GameResources.rabbitAmbient1, GameResources.rabbitAmbient2)).volume(0.3f);
    }

    @Override
    protected SoundSettings getHurtSound() {
        return new SoundSettings(GameResources.rabbitHurt).volume(0.7f);
    }

    @Override
    protected SoundSettings getDeathSound() {
        return new SoundSettings(GameResources.rabbitDeath).volume(0.4f);
    }
}
