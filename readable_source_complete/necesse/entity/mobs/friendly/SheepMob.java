...
extends HusbandryMob {
    public static LootTable lootTable = new LootTable(LootItem.between("rawmutton", 1, 2), LootItem.between("wool", 1, 2));
    ...
    public void init() {
        super.init();
        this.ai = new BehaviourTreeAI<SheepMob>(this, new HusbandryImpregnateWandererAI(30000));
    }
    ...
    protected SoundSettings getAmbientSound() {
        return new SoundSettings(GameResources.sheepAmbient).volume(0.8f);
    }

    @Override
    protected SoundSettings getHurtSound() {
        return new SoundSettings(GameResources.sheepHurt);
    }

    @Override
    protected SoundSettings getDeathSound() {
        return new SoundSettings(GameResources.sheepDeath).volume(0.4f);
    }
    ...
    public void addDrawables(List<MobDrawable> list, OrderableDrawables tileList, OrderableDrawables topList, Level level, int x, int y, TickManager tickManager, GameCamera camera, PlayerMob perspective) {
        ...
        GameLight light = level.getLightLevel(SheepMob.getTileCoordinate(x), SheepMob.getTileCoordinate(y));
        ...
        final TextureDrawOptionsEnd options = this.getTexture().initDraw().sprite(sprite.x, sprite.y, 64).light(light).addMaskShader(swimMask).pos(drawX, drawY += level.getTile(SheepMob.getTileCoordinate(x), SheepMob.getTileCoordinate(y)).getMobSinkingAmount(this));
    ...
    private GameTexture getTexture() {
        if (this.isGrown()) {
            if (this.hasWool()) {
                return MobRegistry.Textures.sheep;
            }
            return MobRegistry.Textures.sheep_sheared;
        }
    ...
    public String getRandomChildMobStringID(HusbandryMob father) {
        return GameRandom.globalRandom.getOneOf(this.getStringID(), "ram");
    }
