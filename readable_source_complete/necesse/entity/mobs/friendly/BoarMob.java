...
extends HusbandryMob {
    ...
    public void init() {
        super.init();
        this.ai = new BehaviourTreeAI<BoarMob>(this, new HusbandryImpregnateWandererAI(30000));
    }
    ...
    public void addDrawables(List<MobDrawable> list, OrderableDrawables tileList, OrderableDrawables topList, Level level, int x, int y, TickManager tickManager, GameCamera camera, PlayerMob perspective) {
        ...
        GameLight light = level.getLightLevel(BoarMob.getTileCoordinate(x), BoarMob.getTileCoordinate(y));
        ...
        final TextureDrawOptionsEnd options = this.getTexture().initDraw().sprite(sprite.x, sprite.y, 64).light(light).addMaskShader(swimMask).pos(drawX, drawY += level.getTile(BoarMob.getTileCoordinate(x), BoarMob.getTileCoordinate(y)).getMobSinkingAmount(this));
    ...
    private GameTexture getTexture() {
        if (this.isGrown()) {
            return MobRegistry.Textures.boar;
        }
    ...
    public boolean canImpregnateMob(HusbandryMob other) {
        return other.getStringID().equals("pig");
    }

    @Override
    protected SoundSettings getAmbientSound() {
        return new SoundSettings(GameResources.boarAmbient).volume(0.5f);
    }

    @Override
    protected SoundSettings getHurtSound() {
        return new SoundSettings(GameResources.boarHurt).volume(0.7f);
    }

    @Override
    protected SoundSettings getDeathSound() {
        return new SoundSettings(GameResources.boarDeath).volume(0.7f);
    }
}
