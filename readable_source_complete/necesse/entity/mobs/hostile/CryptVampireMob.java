...
extends HostileMob {
    ...
    public void init() {
        super.init();
        this.ai = new BehaviourTreeAI<CryptVampireMob>(this, new ConfusedPlayerChaserWandererAI<CryptVampireMob>(null, 512, 384, 40000, true, false){

            @Override
            public boolean attackTarget(CryptVampireMob mob, Mob target) {
                if (mob.canAttack() && !mob.isBat) {
                    mob.attack(target.getX(), target.getY(), false);
                    CryptVampireBoltProjectile projectile = new CryptVampireBoltProjectile(mob.getLevel(), mob, mob.x, mob.y, target.x, target.y, 60.0f, 768, damage, 50);
    ...
    public void addDrawables(List<MobDrawable> list, OrderableDrawables tileList, OrderableDrawables topList, Level level, int x, int y, TickManager tickManager, GameCamera camera, PlayerMob perspective) {
        ...
        GameLight light = level.getLightLevel(CryptVampireMob.getTileCoordinate(x), CryptVampireMob.getTileCoordinate(y));
        int drawX = camera.getDrawX(x) - 22 - 10;
        int drawY = camera.getDrawY(y) - 44 - 7;
        int dir = this.getDir();
        Point sprite = this.getAnimSprite(x, y, dir);
        drawY += this.getBobbing(x, y);
        drawY += level.getTile(CryptVampireMob.getTileCoordinate(x), CryptVampireMob.getTileCoordinate(y)).getMobSinkingAmount(this);
