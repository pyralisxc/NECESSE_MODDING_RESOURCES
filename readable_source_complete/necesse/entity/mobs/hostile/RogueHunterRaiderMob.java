...
public class RogueHunterRaiderMob
extends ItemAttackerRaiderMob {
    public HumanLook look;

    public RogueHunterRaiderMob() {
        super(false);
        this.setSpeed(25.0f);
    ...
    public LootTable getLootTable() {
        String arrow = GameRandom.globalRandom.getOneOf("firearrow", "frostarrow", "poisonarrow", "bouncingarrow", "ironarrow");
        int minCoins = this.getMaxHealth() / 30;
        int maxCoins = this.getMaxHealth() / 20;
        return new LootTable(super.getLootTable(), LootItem.between("coin", minCoins, maxCoins).splitItems(4), LootItem.between(arrow, 6, 16), new ConditionLootItemList((random, objects) -> this.getMaxWeaponOrArmorValue() >= 2000.0, ChanceLootItem.between(0.5f, "spideritearrow", 3, 9)), ChanceLootItem.between(0.1f, "duckbreast", 2, 5), ChanceLootItem.between(0.1f, "rabbitleg", 2, 5), new ChanceLootItem(0.05f, "accuracypotion"), new ConditionLootItemList((random, objects) -> this.getMaxWeaponOrArmorValue() >= 2000.0, new ChanceLootItem(0.05f, "greateraccuracypotion")), new ChanceLootItem(0.05f, "rapidpotion"), new ConditionLootItemList((random, objects) -> this.getMaxWeaponOrArmorValue() >= 2000.0, new ChanceLootItem(0.05f, "greaterrapidpotion")), new ChanceLootItem(0.02f, "rangerpotion"), new ChanceLootItem(0.02f, "fuzzydice"), new ChanceLootItem(0.02f, "hunterhoodmask"), new ChanceLootItem(0.01f, "strikebanner"));
    }
    ...
    public void addDrawables(List<MobDrawable> list, OrderableDrawables tileList, OrderableDrawables topList, Level level, int x, int y, TickManager tickManager, GameCamera camera, PlayerMob perspective) {
        ...
        GameLight light = level.getLightLevel(RogueHunterRaiderMob.getTileCoordinate(x), RogueHunterRaiderMob.getTileCoordinate(y));
        ...
        drawY += level.getTile(RogueHunterRaiderMob.getTileCoordinate(x), RogueHunterRaiderMob.getTileCoordinate(y)).getMobSinkingAmount(this);
        ...
        if (this.helmet != null) {
            humanOptions.helmet(new InventoryItem("hunterhoodmask"));
        }
        if (this.chest != null) {
            humanOptions.chestplate(new InventoryItem("huntershirt"));
        }
        if (this.boots != null) {
            humanOptions.boots(new InventoryItem("hunterboots"));
        }
    ...
    public void drawOnMap(TickManager tickManager, Client client, int x, int y, double tileScale, Rectangle drawBounds, boolean isMinimap) {
        ...
        HumanDrawOptions humanOptions = new HumanDrawOptions(this.getLevel(), this.look, false).helmet(new InventoryItem("hunterhoodmask")).sprite(sprite).dir(dir).size(32, 32);
