...
extends ItemAttackerRaiderMob {
    ...
    public LootTable getLootTable() {
        ...
        return new LootTable(super.getLootTable(), LootItem.between("coin", minCoins, maxCoins).splitItems(4));
    }

    @Override
    public void updateAIAndLook() {
        super.updateAIAndLook();
        this.look = new HumanLook(new GameRandom(this.lookSeed), true);
    }
    ...
    public void addDrawables(List<MobDrawable> list, OrderableDrawables tileList, OrderableDrawables topList, Level level, int x, int y, TickManager tickManager, GameCamera camera, PlayerMob perspective) {
        ...
        GameLight light = level.getLightLevel(HumanRaiderMob.getTileCoordinate(x), HumanRaiderMob.getTileCoordinate(y));
        ...
        drawY += level.getTile(HumanRaiderMob.getTileCoordinate(x), HumanRaiderMob.getTileCoordinate(y)).getMobSinkingAmount(this);
        MaskShaderOptions swimMask = this.getSwimMaskShaderOptions(this.inLiquidFloat(x, y));
        HumanDrawOptions humanOptions = new HumanDrawOptions(level, this.look, true).sprite(sprite).mask(swimMask).dir(dir).light(light);
        if (inLiquid) {
