...
public class AncientSkeletonMob
extends HostileMob {
    public static LootTable lootTable = new LootTable(LootItem.between("bone", 1, 3));
    public static GameDamage baseDamage = new GameDamage(100.0f);
    ...
    public void init() {
        ...
        this.ai = new BehaviourTreeAI<AncientSkeletonMob>(this, new ConfusedCollisionPlayerChaserWandererAI(null, 512, damage, 100, 40000));
    ...
    public void addDrawables(List<MobDrawable> list, OrderableDrawables tileList, OrderableDrawables topList, Level level, int x, int y, TickManager tickManager, GameCamera camera, PlayerMob perspective) {
        ...
        GameLight light = level.getLightLevel(AncientSkeletonMob.getTileCoordinate(x), AncientSkeletonMob.getTileCoordinate(y));
        int drawX = camera.getDrawX(x) - 22 - 10;
        int drawY = camera.getDrawY(y) - 44 - 7;
        int dir = this.getDir();
        Point sprite = this.getAnimSprite(x, y, dir);
        drawY += this.getBobbing(x, y);
        drawY += level.getTile(AncientSkeletonMob.getTileCoordinate(x), AncientSkeletonMob.getTileCoordinate(y)).getMobSinkingAmount(this);
