...
public class FrozenDwarfRaiderMob
extends ItemAttackerRaiderMob {
    public FrozenDwarfRaiderMob() {
        super(false);
        this.setSpeed(25.0f);
    ...
    public LootTable getLootTable() {
        ...
        return new LootTable(super.getLootTable(), LootItem.between("coin", minCoins, maxCoins).splitItems(4), new ChanceLootItem(0.15f, "healthpotion"), new ConditionLootItemList((random, objects) -> this.getMaxWeaponOrArmorValue() >= 1150.0, new ChanceLootItem(0.1f, "greaterhealthpotion")), new ConditionLootItemList((random, objects) -> this.getMaxWeaponOrArmorValue() >= 2000.0, new ChanceLootItem(0.05f, "superiorhealthpotion")), new ChanceLootItem(0.05f, "speedpotion"), new ConditionLootItemList((random, objects) -> this.getMaxWeaponOrArmorValue() >= 2000.0, new ChanceLootItem(0.05f, "greaterspeedpotion")), new ChanceLootItem(0.05f, "resistancepotion"), new ConditionLootItemList((random, objects) -> this.getMaxWeaponOrArmorValue() >= 2000.0, new ChanceLootItem(0.05f, "greaterresistancepotion")), ChanceLootItem.between(0.1f, "frostshard", 2, 6), new ConditionLootItemList((random, objects) -> this.getMaxWeaponOrArmorValue() >= 1450.0, ChanceLootItem.between(0.05f, "glacialshard", 1, 5)), new ChanceLootItem(0.01f, "frostpickaxe"), new ConditionLootItemList((random, objects) -> this.getMaxWeaponOrArmorValue() >= 1450.0, new ChanceLootItem(0.01f, "glacialpickaxe")), new ChanceLootItem(0.01f, "frozenwave"), new ChanceLootItem(0.01f, "frozenheart"), new ChanceLootItem(0.01f, "calmingrose"), new ChanceLootItem(0.01f, "sparegemstones"));
    }
    ...
    public void spawnDeathParticles(float knockbackX, float knockbackY) {
        GameTexture bodyTexture = MobRegistry.Textures.frozenDwarf.body;
        for (int i = 0; i < 4; ++i) {
    ...
    public void addDrawables(List<MobDrawable> list, OrderableDrawables tileList, OrderableDrawables topList, Level level, int x, int y, TickManager tickManager, GameCamera camera, PlayerMob perspective) {
        ...
        GameLight light = level.getLightLevel(FrozenDwarfRaiderMob.getTileCoordinate(x), FrozenDwarfRaiderMob.getTileCoordinate(y));
        ...
        drawY += level.getTile(FrozenDwarfRaiderMob.getTileCoordinate(x), FrozenDwarfRaiderMob.getTileCoordinate(y)).getMobSinkingAmount(this);
        MaskShaderOptions swimMask = this.getSwimMaskShaderOptions(this.inLiquidFloat(x, y));
        HumanDrawOptions humanOptions = new HumanDrawOptions(level, MobRegistry.Textures.frozenDwarf).sprite(sprite).mask(swimMask).dir(dir).light(light);
        if (inLiquid) {
    ...
    public void drawOnMap(TickManager tickManager, Client client, int x, int y, double tileScale, Rectangle drawBounds, boolean isMinimap) {
        ...
        HumanDrawOptions humanOptions = new HumanDrawOptions(this.getLevel(), MobRegistry.Textures.frozenDwarf).sprite(sprite).dir(dir).size(32, 32);
