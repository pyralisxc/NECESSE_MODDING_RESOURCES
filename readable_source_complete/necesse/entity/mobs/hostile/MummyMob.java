...
extends HostileMob {
    ...
    public void init() {
        super.init();
        this.ai = new BehaviourTreeAI<MummyMob>(this, new ConfusedCollisionPlayerChaserWandererAI(() -> !this.getLevel().isCave && !this.getWorldEntity().isNight() && this.canDespawn, 384, new GameDamage(48.0f), 100, 40000));
    ...
    public void addDrawables(List<MobDrawable> list, OrderableDrawables tileList, OrderableDrawables topList, Level level, int x, int y, TickManager tickManager, GameCamera camera, PlayerMob perspective) {
        ...
        GameLight light = level.getLightLevel(MummyMob.getTileCoordinate(x), MummyMob.getTileCoordinate(y));
        int drawX = camera.getDrawX(x) - 22 - 10;
        int drawY = camera.getDrawY(y) - 44 - 7;
        int dir = this.getDir();
        Point sprite = this.getAnimSprite(x, y, dir);
        drawY += this.getBobbing(x, y);
        drawY += level.getTile(MummyMob.getTileCoordinate(x), MummyMob.getTileCoordinate(y)).getMobSinkingAmount(this);
