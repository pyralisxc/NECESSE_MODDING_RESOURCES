...
public class MummyRaiderMob
extends ItemAttackerRaiderMob {
    public MummyRaiderMob() {
        super(false);
        this.setSpeed(25.0f);
    ...
    public LootTable getLootTable() {
        ...
        return new LootTable(super.getLootTable(), LootItem.between("coin", minCoins, maxCoins).splitItems(4), new ChanceLootItem(0.15f, "healthpotion"), new ConditionLootItemList((random, objects) -> this.getMaxWeaponOrArmorValue() >= 1150.0, new ChanceLootItem(0.1f, "greaterhealthpotion")), new ConditionLootItemList((random, objects) -> this.getMaxWeaponOrArmorValue() >= 2000.0, new ChanceLootItem(0.05f, "superiorhealthpotion")), new ChanceLootItem(0.15f, "thornspotion"), ChanceLootItem.between(0.1f, "quartz", 2, 6), new ConditionLootItemList((random, objects) -> this.getMaxWeaponOrArmorValue() >= 1750.0, ChanceLootItem.between(0.05f, "ancientfossilore", 1, 5)), new ChanceLootItem(0.05f, "enchantingscroll"), new ChanceLootItem(0.02f, "spelunkerpotion"), new ChanceLootItem(0.02f, "strengthpotion"), new ChanceLootItem(0.01f, "ancientfeather"), new ChanceLootItem(0.01f, "miningcharm"), new ChanceLootItem(0.01f, "prophecyslab"), new ChanceLootItem(0.01f, "airvessel"));
    }
    ...
    public void spawnDeathParticles(float knockbackX, float knockbackY) {
        GameTexture bodyTexture = MobRegistry.Textures.mummy.body;
        for (int i = 0; i < 4; ++i) {
    ...
    public void addDrawables(List<MobDrawable> list, OrderableDrawables tileList, OrderableDrawables topList, Level level, int x, int y, TickManager tickManager, GameCamera camera, PlayerMob perspective) {
        ...
        GameLight light = level.getLightLevel(MummyRaiderMob.getTileCoordinate(x), MummyRaiderMob.getTileCoordinate(y));
        ...
        drawY += level.getTile(MummyRaiderMob.getTileCoordinate(x), MummyRaiderMob.getTileCoordinate(y)).getMobSinkingAmount(this);
        MaskShaderOptions swimMask = this.getSwimMaskShaderOptions(this.inLiquidFloat(x, y));
        HumanDrawOptions humanOptions = new HumanDrawOptions(level, MobRegistry.Textures.mummy).sprite(sprite).mask(swimMask).dir(dir).light(light);
        if (inLiquid) {
    ...
    public void drawOnMap(TickManager tickManager, Client client, int x, int y, double tileScale, Rectangle drawBounds, boolean isMinimap) {
        ...
        HumanDrawOptions humanOptions = new HumanDrawOptions(this.getLevel(), MobRegistry.Textures.mummy).sprite(sprite).dir(dir).size(32, 32);
