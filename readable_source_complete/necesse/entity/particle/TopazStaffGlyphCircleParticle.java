...
public class TopazStaffGlyphCircleParticle
extends Particle {
    public int sprite = GameRandom.globalRandom.nextInt(4);
    private float alpha = 0.0f;

    public TopazStaffGlyphCircleParticle(Level level, float x, float y, long lifeTime) {
    ...
    public void addDrawables(List<LevelSortedDrawable> list, OrderableDrawables tileList, OrderableDrawables topList, OrderableDrawables overlayList, Level level, TickManager tickManager, GameCamera camera, PlayerMob perspective) {
        ...
        TextureDrawOptionsEnd options = GameResources.topazGlyphCircleParticle.initDraw().light(light).alpha(this.alpha).pos(drawX, drawY).posMiddle(drawX, drawY);
