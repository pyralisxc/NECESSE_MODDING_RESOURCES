...
extends Particle {
    ...
    public AfterimageParticle(Level level, PlayerMob player) {
    ...
    private InventoryItem getArmor(PlayerMob player, int index) {
        InventoryItem invItem = player.getInv().equipment.getSelectedCosmeticSlot(index).getItem();
        if (invItem == null) {
            invItem = player.getInv().equipment.getSelectedArmorSlot(index).getItem();
        }
