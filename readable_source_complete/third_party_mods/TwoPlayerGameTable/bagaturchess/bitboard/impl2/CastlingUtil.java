...
public final class CastlingUtil {
    ...
    public static boolean isValidCastlingMove(ChessBoard cb, int fromIndex, int toIndex) {
        ...
        if (toIndex == 1) {
            ...
            bb_all_pieces_no_king_no_rook = cb.all_pieces & ((ChessConstants.POWER_LOOKUP[cb.castling_config.from_SquareID_king_w] | ChessConstants.POWER_LOOKUP[cb.castling_config.from_SquareID_rook_kingside_w]) ^ 0xFFFFFFFFFFFFFFFFL);
            if ((ChessConstants.POWER_LOOKUP[cb.castling_config.from_SquareID_king_w] & cb.getPieces(0, 6)) == 0L) {
                throw new IllegalStateException();
            }
            if ((ChessConstants.POWER_LOOKUP[cb.castling_config.from_SquareID_rook_kingside_w] & cb.getPieces(0, 4)) == 0L) {
                throw new IllegalStateException();
            }
        } else if (toIndex == 5) {
            ...
            bb_all_pieces_no_king_no_rook = cb.all_pieces & ((ChessConstants.POWER_LOOKUP[cb.castling_config.from_SquareID_king_w] | ChessConstants.POWER_LOOKUP[cb.castling_config.from_SquareID_rook_queenside_w]) ^ 0xFFFFFFFFFFFFFFFFL);
            if ((ChessConstants.POWER_LOOKUP[cb.castling_config.from_SquareID_king_w] & cb.getPieces(0, 6)) == 0L) {
                throw new IllegalStateException();
            }
            if ((ChessConstants.POWER_LOOKUP[cb.castling_config.from_SquareID_rook_queenside_w] & cb.getPieces(0, 4)) == 0L) {
                throw new IllegalStateException();
            }
        } else if (toIndex == 57) {
            ...
            bb_all_pieces_no_king_no_rook = cb.all_pieces & ((ChessConstants.POWER_LOOKUP[cb.castling_config.from_SquareID_king_b] | ChessConstants.POWER_LOOKUP[cb.castling_config.from_SquareID_rook_kingside_b]) ^ 0xFFFFFFFFFFFFFFFFL);
            if ((ChessConstants.POWER_LOOKUP[cb.castling_config.from_SquareID_king_b] & cb.getPieces(1, 6)) == 0L) {
                throw new IllegalStateException();
            }
            if ((ChessConstants.POWER_LOOKUP[cb.castling_config.from_SquareID_rook_kingside_b] & cb.getPieces(1, 4)) == 0L) {
                throw new IllegalStateException();
            }
        } else if (toIndex == 61) {
            ...
            bb_all_pieces_no_king_no_rook = cb.all_pieces & ((ChessConstants.POWER_LOOKUP[cb.castling_config.from_SquareID_king_b] | ChessConstants.POWER_LOOKUP[cb.castling_config.from_SquareID_rook_queenside_b]) ^ 0xFFFFFFFFFFFFFFFFL);
            if ((ChessConstants.POWER_LOOKUP[cb.castling_config.from_SquareID_king_b] & cb.getPieces(1, 6)) == 0L) {
                throw new IllegalStateException();
            }
            if ((ChessConstants.POWER_LOOKUP[cb.castling_config.from_SquareID_rook_queenside_b] & cb.getPieces(1, 4)) == 0L) {
                throw new IllegalStateException();
            }
        } else {
        ...
        int n = king_color = (ChessConstants.POWER_LOOKUP[fromIndex] & cb.getPieces(0, 6)) != 0L ? 0 : 1;
        while (bb_KingInBetween != 0L) {
            int intermediate_square_id = Long.numberOfTrailingZeros(bb_KingInBetween);
            if (CastlingUtil.isInCheckIncludingKing(intermediate_square_id, king_color, cb, bb_all_pieces_no_king_no_rook)) {
                return false;
            }
    ...
    private static boolean isInCheckIncludingKing(int kingIndex, int colorToMove, ChessBoard cb, long allPieces) {
        return (cb.getPieces(1 - colorToMove, 2) & ChessConstants.KNIGHT_MOVES[kingIndex] | (cb.getPieces(1 - colorToMove, 4) | cb.getPieces(1 - colorToMove, 5)) & MagicUtil.getRookMoves(kingIndex, allPieces) | (cb.getPieces(1 - colorToMove, 3) | cb.getPieces(1 - colorToMove, 5)) & MagicUtil.getBishopMoves(kingIndex, allPieces) | cb.getPieces(1 - colorToMove, 1) & ChessConstants.PAWN_ATTACKS[colorToMove][kingIndex] | cb.getPieces(1 - colorToMove, 6) & ChessConstants.KING_MOVES[kingIndex]) != 0L;
    }
