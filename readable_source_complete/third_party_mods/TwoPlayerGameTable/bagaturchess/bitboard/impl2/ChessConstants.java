...
public class ChessConstants {
    ...
    public static final long[] POWER_LOOKUP;
    ...
    static {
        ...
        int currentPosition;
        int i2;
        POWER_LOOKUP = new long[64];
        for (i2 = 0; i2 < 64; ++i2) {
            ChessConstants.POWER_LOOKUP[i2] = 1L << i2;
        }
        ...
        for (currentPosition = 0; currentPosition < 64; ++currentPosition) {
            for (newPosition = 0; newPosition < 64; ++newPosition) {
                if (newPosition == currentPosition + 7 && newPosition % 8 != 7) {
                    ...
                    lArray[n] = lArray[n] | POWER_LOOKUP[newPosition];
                }
        ...
        for (currentPosition = 0; currentPosition < 64; ++currentPosition) {
            for (newPosition = 0; newPosition < 64; ++newPosition) {
                if (!ChessConstants.isKnightMove(currentPosition, newPosition)) continue;
                int n = currentPosition;
                KNIGHT_MOVES[n] = KNIGHT_MOVES[n] | POWER_LOOKUP[newPosition];
            }
        }
        for (currentPosition = 0; currentPosition < 64; ++currentPosition) {
            for (newPosition = 0; newPosition < 64; ++newPosition) {
                if (!ChessConstants.isKingMove(currentPosition, newPosition)) continue;
                int n = currentPosition;
                KING_MOVES[n] = KING_MOVES[n] | POWER_LOOKUP[newPosition];
            }
        }
            ...
            for (to = from + 1; to < 64; ++to) {
                if (from / 8 == to / 8) {
                    for (i2 = to - 1; i2 > from; --i2) {
                        ...
                        int n = to;
                        lArray[n] = lArray[n] | POWER_LOOKUP[i2];
                    }
                }
        ...
        for (int pinnedPieceIndex = 0; pinnedPieceIndex < 64; ++pinnedPieceIndex) {
            for (int kingIndex = 0; kingIndex < 64; ++kingIndex) {
                ...
                block21: for (int direction : DIRECTION) {
                ...
                for (int xray = kingIndex + correctDirection; xray >= 0 && xray < 64 && (correctDirection != -1 && correctDirection != -9 && correctDirection != 7 || (xray & 7) != 7) && (correctDirection != 1 && correctDirection != 9 && correctDirection != -7 || (xray & 7) != 0); xray += correctDirection) {
                    ...
                    int n = kingIndex;
                    lArray[n] = lArray[n] | POWER_LOOKUP[xray];
                }
            }
        }
        for (i = 0; i < 64; ++i) {
            ...
            int n = i;
            lArray[n] = lArray[n] | (KING_MOVES[i] | POWER_LOOKUP[i]);
            long[] lArray2 = KING_AREA[1];
            int n2 = i;
            lArray2[n2] = lArray2[n2] | (KING_MOVES[i] | POWER_LOOKUP[i]);
            if (i > 15) {
                long[] lArray3 = KING_AREA[1];
                int n3 = i;
                lArray3[n3] = lArray3[n3] | KING_MOVES[i] >>> 8;
            }
            if (i >= 48) continue;
            long[] lArray4 = KING_AREA[0];
            int n4 = i;
            lArray4[n4] = lArray4[n4] | KING_MOVES[i] << 8;
        }
        for (i = 0; i < 64; ++i) {
