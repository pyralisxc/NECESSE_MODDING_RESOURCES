...
public class BlackPawnsChecks
extends BlackPawnPlies {
    ...
    public static void initPair_Attacks_Dynamic(int fromFieldID, int toFieldID) {
        int[] fromFieldValidDirIDs = ALL_BLACK_PAWN_ATTACKS_VALID_DIRS[fromFieldID];
        int[][] middleFieldIDs = ALL_BLACK_PAWN_ATTACKS_DIRS_WITH_FIELD_IDS[fromFieldID];
        ...
        for (int dirID1 : fromFieldValidDirIDs) {
            ...
            for (int seq1 = 0; seq1 < middleFieldIDsByDir.length; ++seq1) {
                ...
                int[] middleFieldValidDirIDs = ALL_BLACK_PAWN_ATTACKS_VALID_DIRS[middleFieldID];
                int[][] toFieldIDs = ALL_BLACK_PAWN_ATTACKS_DIRS_WITH_FIELD_IDS[middleFieldID];
                long[][] toFieldBitboards = ALL_BLACK_PAWN_ATTACKS_DIRS_WITH_BITBOARDS[middleFieldID];
                    ...
                    for (int seq2 = 0; seq2 < cur_toFieldIDsByDir.length; ++seq2) {
                        ...
                        if (toFieldID == cur_toFieldID) {
                            ...
                            if (CHECK_ATTACK_MIDDLE_FIELDS_IDS[fromFieldID][toFieldID] == null || CHECK_ATTACK_MIDDLE_FIELDS_IDS[fromFieldID][toFieldID].length <= 1) {
                                BlackPawnsChecks.CHECK_ATTACK_MIDDLE_FIELDS_IDS[fromFieldID][toFieldID] = BlackPawnsChecks.extendArray(CHECK_ATTACK_MIDDLE_FIELDS_IDS[fromFieldID][toFieldID], middleFieldID);
                                BlackPawnsChecks.CHECK_ATTACK_MIDDLE_FIELDS_DIR_ID[fromFieldID][toFieldID] = BlackPawnsChecks.extendArray(CHECK_ATTACK_MIDDLE_FIELDS_DIR_ID[fromFieldID][toFieldID], dirID1);
                                BlackPawnsChecks.CHECK_ATTACK_MIDDLE_FIELDS_SEQS[fromFieldID][toFieldID] = BlackPawnsChecks.extendArray(CHECK_ATTACK_MIDDLE_FIELDS_SEQS[fromFieldID][toFieldID], seq1);
                                BlackPawnsChecks.CHECK_ATTACK_MIDDLE_FIELDS_BITBOARDS[fromFieldID][toFieldID] = BlackPawnsChecks.extendArray(CHECK_ATTACK_MIDDLE_FIELDS_BITBOARDS[fromFieldID][toFieldID], middleFieldBitboard);
                                continue block2;
                            }
    ...
    public static void genAll_Attacks_Dynamic() {
        for (int i = 0; i < ALL_ORDERED_A1H1.length; ++i) {
            int fromID = BlackPawnsChecks.get67IDByBitboard(ALL_ORDERED_A1H1[i]);
            for (int j = 0; j < ALL_ORDERED_A1H1.length; ++j) {
                int toID = BlackPawnsChecks.get67IDByBitboard(ALL_ORDERED_A1H1[j]);
                if (fromID == toID) continue;
                BlackPawnsChecks.initPair_Attacks_Dynamic(fromID, toID);
            }
        }
    }

    public static void initPair_NonAttacks_Dynamic(int fromFieldID, int toFieldID) {
        int[] fromFieldValidDirIDs = ALL_BLACK_PAWN_NONATTACKS_VALID_DIRS[fromFieldID];
        int[][] middleFieldIDs = ALL_BLACK_PAWN_NONATTACKS_DIRS_WITH_FIELD_IDS[fromFieldID];
                    ...
                    for (int seq2 = 0; seq2 < cur_toFieldIDsByDir.length; ++seq2) {
                        ...
                        if (toFieldID == cur_toFieldID) {
                            ...
                            if (CHECK_NONATTACK_MIDDLE_FIELDS_IDS[fromFieldID][toFieldID] == null || CHECK_NONATTACK_MIDDLE_FIELDS_IDS[fromFieldID][toFieldID].length <= 1) {
                                BlackPawnsChecks.CHECK_NONATTACK_MIDDLE_FIELDS_IDS[fromFieldID][toFieldID] = BlackPawnsChecks.extendArray(CHECK_NONATTACK_MIDDLE_FIELDS_IDS[fromFieldID][toFieldID], middleFieldID);
                                BlackPawnsChecks.CHECK_NONATTACK_MIDDLE_FIELDS_DIR_ID[fromFieldID][toFieldID] = BlackPawnsChecks.extendArray(CHECK_NONATTACK_MIDDLE_FIELDS_DIR_ID[fromFieldID][toFieldID], dirID1);
                                BlackPawnsChecks.CHECK_NONATTACK_MIDDLE_FIELDS_SEQS[fromFieldID][toFieldID] = BlackPawnsChecks.extendArray(CHECK_NONATTACK_MIDDLE_FIELDS_SEQS[fromFieldID][toFieldID], seq1);
                                BlackPawnsChecks.CHECK_NONATTACK_MIDDLE_FIELDS_BITBOARDS[fromFieldID][toFieldID] = BlackPawnsChecks.extendArray(CHECK_NONATTACK_MIDDLE_FIELDS_BITBOARDS[fromFieldID][toFieldID], middleFieldBitboard);
                                continue block2;
                            }
                            throw new IllegalStateException();
                        }
    ...
    public static void genAll_NonAttacks_Dynamic() {
        for (int i = 0; i < ALL_ORDERED_A1H1.length; ++i) {
            ...
            for (int j = 0; j < ALL_ORDERED_A1H1.length; ++j) {
                int toID = BlackPawnsChecks.get67IDByBitboard(ALL_ORDERED_A1H1[j]);
                if (fromID == toID) continue;
                BlackPawnsChecks.initPair_NonAttacks_Dynamic(fromID, toID);
            }
        }
    }
    ...
    public static String testAttackChecks(int fromFieldID, int toFieldID) {
        int i;
        String result = "White pawn atack checks from " + BlackPawnsChecks.getFieldSign_UC(fromFieldID) + " to " + BlackPawnsChecks.getFieldSign_UC(toFieldID) + " -> ";
        ...
        if (fields == null) {
            ...
            for (i = 0; i < fields.length; ++i) {
                result = result + BlackPawnsChecks.getFieldSign_UC(fields[i]) + ", ";
            }
            if (fields != null) {
                result = result + "\r\n";
                for (i = 0; i < fields.length; ++i) {
    ...
    public static String testNonAttackChecks(int fromFieldID, int toFieldID) {
        int i;
        String result = "White pawn nonattack checks from " + BlackPawnsChecks.getFieldSign_UC(fromFieldID) + " to " + BlackPawnsChecks.getFieldSign_UC(toFieldID) + " -> ";
    ...
    public static void testAll() {
        for (int i = 0; i < ALL_ORDERED_A1H1.length; ++i) {
            ...
            for (int j = 0; j < ALL_ORDERED_A1H1.length; ++j) {
                int toID = BlackPawnsChecks.get67IDByBitboard(ALL_ORDERED_A1H1[j]);
                BlackPawnsChecks.testAttackChecks(fromID, toID);
                BlackPawnsChecks.testNonAttackChecks(fromID, toID);
            }
        }
    }

    public static void main(String[] args) {
        int from = BlackPawnsChecks.get67IDByBitboard(0x8000000000000L);
        int to = BlackPawnsChecks.get67IDByBitboard(0x8000000L);
        BlackPawnsChecks.testAll();
    }

    static {
        try {
            BlackPawnsChecks.genAll_Attacks_Dynamic();
            BlackPawnsChecks.genAll_NonAttacks_Dynamic();
        }
        catch (Throwable t) {
            t.printStackTrace();
        }
