...
extends PSTGen {
    public static int genAttacks(int colour, int fromFieldID, long opPawnsAttacks, int opColour, long myKing, long opKing, long myPawns, long opPawns, PawnsModel model, IBitBoard bitboard) {
        ...
        int attack_king = 0;
        long[][] dirs = CastlePlies.ALL_CASTLE_DIRS_WITH_BITBOARDS[fromFieldID];
        int[][] dirFieldIDs = CastlePlies.ALL_CASTLE_DIRS_WITH_FIELD_IDS[fromFieldID];
        block0: for (int dirID : CastlePlies.ALL_CASTLE_VALID_DIRS[fromFieldID]) {
    ...
    public static void fillPST(int[] pst, int colour, PawnsModel model, IBitBoard bitboard) {
        ...
        for (int fromFieldID = 0; fromFieldID < 64; ++fromFieldID) {
            ...
            pst[fromFieldID] = (opPawnsAttacks & fromBitboard) != 0L ? PENALTY_ON_ATTACKED_SQUARE : PSTGenRooks.genAttacks(colour, fromFieldID, opPawnsAttacks, opColour, myKing, opKing, myPawns, opPawns, model, bitboard);
        }
    }
    ...
    public static void main(String[] args) {
        ...
        int[] pst = new int[64];
        PSTGenRooks.fillPST(pst, 0, ModelBuilder.build(bitboard), bitboard);
