...
public class HashMapLongLong {
    ...
    public static final long NO_VALUE = -1000000000L;
    ...
    public boolean contains(int value) {
        for (int i = 0; i < this.capacity; ++i) {
            int pos = this.nextPtr[i];
            while (pos != -1) {
                if (this.elements[pos - this.capacity] == (long)value) {
                    return true;
                }
                pos = this.nextPtr[pos];
            }
    ...
    public boolean containsKey(long key) {
        int pos = this.nextPtr[HashMapLongLong.hash(key) % this.capacity];
        while (pos != -1) {
    ...
    public boolean equals(Object object) {
        ...
        for (int i = 0; i < this.capacity; ++i) {
            ...
            while (pos != -1) {
                ...
                long temp = t.get(this.keys[index]);
                if (temp == -1000000000L) {
                    return false;
                }
    ...
    public boolean put(long key, long value) {
        ...
        int pos = HashMapLongLong.hash(key) % this.capacity;
    ...
    public long dec(long key) {
        int prevPos = HashMapLongLong.hash(key) % this.capacity;
        int pos = this.nextPtr[prevPos];
        while (pos != -1) {
            int index = pos - this.capacity;
            if (this.keys[index] == key) {
                int n = index;
                this.elements[n] = this.elements[n] - 1L;
                if (this.elements[index] == 0L) {
    ...
    protected void init(int initialCapacity) {
        ...
        this.elements = new long[this.limit];
        this.nextFree = this.capacity;
        this.count = 0;
    }
    ...
    private void writeObject(ObjectOutputStream stream) throws IOException {
        ...
        for (int i = 0; i < this.capacity; ++i) {
            ...
            while (pos != -1) {
                ...
                stream.writeLong(this.elements[index]);
                pos = this.nextPtr[pos];
            }
        }
    }
