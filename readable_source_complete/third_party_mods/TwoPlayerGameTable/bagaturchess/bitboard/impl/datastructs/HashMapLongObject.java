...
public class HashMapLongObject<V> {
    ...
    protected transient V[] elements;
    ...
    public void visitValues(IValuesVisitor_HashMapLongObject<V> visitor) {
        for (int i = 0; i < this.capacity; ++i) {
            int pos = this.nextPtr[i];
            while (pos != -1) {
                V value = this.elements[pos - this.capacity];
                visitor.visit(value);
                pos = this.nextPtr[pos];
            }
        }
    ...
    public boolean containsKey(long key) {
        int pos = this.nextPtr[HashMapLongObject.hash(key) % this.capacity];
        while (pos != -1) {
    ...
    public boolean put(long key, V value) {
        ...
        ++puts;
        if (value == null) {
            throw new IllegalStateException("value=" + String.valueOf(value));
        }
        if (this.count == this.limit) {
            this.rehash();
        }
        int pos = HashMapLongObject.hash(key) % this.capacity;
    ...
    public V get(long key) {
        int pos = this.nextPtr[HashMapLongObject.hash(key) % this.capacity];
        if (pos < 0 && pos != -1) {
            throw new IllegalStateException("pos=" + pos + ", capacity=" + this.capacity + ", key=" + key + ", hash(key)=" + HashMapLongObject.hash(key));
        }
        while (pos != -1) {
            int index = pos - this.capacity;
            if (index < 0) {
                throw new IllegalStateException("index=" + index + ", pos=" + pos + ", capacity=" + this.capacity + ", key=" + key + ", hash(key)=" + HashMapLongObject.hash(key));
            }
    ...
    public boolean remove(long key) {
        int prevPos = HashMapLongObject.hash(key) % this.capacity;
    ...
    public String toString() {
        ...
        for (int i = 0; i < this.capacity; ++i) {
            ...
            while (pos != -1) {
                int index = pos - this.capacity;
                buf.append(this.keys[index] + "=" + String.valueOf(this.elements[index]));
                if (++c < this.count) {
                    buf.append(", ");
                }
                pos = this.nextPtr[pos];
    ...
    private void readObject(ObjectInputStream stream) throws IOException, ClassNotFoundException {
        ...
        for (int i2 = 0; i2 < size; ++i2) {
            this.putQuick(stream.readLong(), stream.readObject());
        }
    }
