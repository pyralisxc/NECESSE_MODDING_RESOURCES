...
public class Util {
    ...
    public static int getDistance(long sq1, long sq2) {
        return Math.max(Math.abs((Long.numberOfTrailingZeros(sq1) >>> 3) - (Long.numberOfTrailingZeros(sq2) >>> 3)), Math.abs((Long.numberOfTrailingZeros(sq1) & 7) - (Long.numberOfTrailingZeros(sq2) & 7)));
    }

    static {
        for (int i = 0; i < 64; ++i) {
            Util.POWER_LOOKUP[i] = 1L << i;
        }
    }
}
