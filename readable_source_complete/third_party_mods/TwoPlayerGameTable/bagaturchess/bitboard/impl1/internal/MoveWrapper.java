...
public class MoveWrapper {
    ...
    public MoveWrapper(String moveString, ChessBoard cb, boolean isFRC) {
        if ("O-O".equals(moveString)) {
            this.isCastling = true;
            this.fromIndex = cb.colorToMove == 0 ? cb.castlingConfig.from_SquareID_king_w : cb.castlingConfig.from_SquareID_king_b;
            this.toIndex = cb.colorToMove == 0 ? 1 : 57;
            this.fromFile = (char)(104 - this.fromIndex % 8);
            this.fromRank = this.fromIndex / 8 + 1;
            this.toFile = (char)(104 - this.toIndex % 8);
            this.toRank = this.toIndex / 8 + 1;
            ...
            this.isCastling = true;
            this.fromIndex = cb.colorToMove == 0 ? cb.castlingConfig.from_SquareID_king_w : cb.castlingConfig.from_SquareID_king_b;
            this.toIndex = cb.colorToMove == 0 ? 5 : 61;
            this.fromFile = (char)(104 - this.fromIndex % 8);
            this.fromRank = this.fromIndex / 8 + 1;
            this.toFile = (char)(104 - this.toIndex % 8);
            this.toRank = this.toIndex / 8 + 1;
        ...
        int n = (cb.pieces[cb.colorToMove][1] & Util.POWER_LOOKUP[this.fromIndex]) != 0L ? 1 : ((cb.pieces[cb.colorToMove][3] & Util.POWER_LOOKUP[this.fromIndex]) != 0L ? 3 : ((cb.pieces[cb.colorToMove][2] & Util.POWER_LOOKUP[this.fromIndex]) != 0L ? 2 : ((cb.pieces[cb.colorToMove][6] & Util.POWER_LOOKUP[this.fromIndex]) != 0L ? 6 : ((cb.pieces[cb.colorToMove][5] & Util.POWER_LOOKUP[this.fromIndex]) != 0L ? 5 : (this.pieceIndex = (cb.pieces[cb.colorToMove][4] & Util.POWER_LOOKUP[this.fromIndex]) != 0L ? 4 : -1)))));
        if (this.pieceIndex == -1) {
            throw new RuntimeException("Source piece not found at index , cb.pieces[cb.colorToMove][type]=" + cb.pieces[cb.colorToMove][2] + ", fromIndex=" + this.fromIndex + ", cb.colorToMove=" + cb.colorToMove + ", move=" + moveString + ", board=" + cb.toString());
        }
        int n2 = (cb.pieces[cb.colorToMoveInverse][1] & Util.POWER_LOOKUP[this.toIndex]) != 0L ? 1 : ((cb.pieces[cb.colorToMoveInverse][3] & Util.POWER_LOOKUP[this.toIndex]) != 0L ? 3 : ((cb.pieces[cb.colorToMoveInverse][2] & Util.POWER_LOOKUP[this.toIndex]) != 0L ? 2 : ((cb.pieces[cb.colorToMoveInverse][6] & Util.POWER_LOOKUP[this.toIndex]) != 0L ? 6 : ((cb.pieces[cb.colorToMoveInverse][5] & Util.POWER_LOOKUP[this.toIndex]) != 0L ? 5 : (this.pieceIndexAttacked = (cb.pieces[cb.colorToMoveInverse][4] & Util.POWER_LOOKUP[this.toIndex]) != 0L ? 4 : 0)))));
            ...
            if (this.pieceIndex == 1 && (this.toRank == 1 || this.toRank == 8)) {
                ...
                if (!this.isCastling && this.pieceIndex == 6) {
                    ...
                    if (!isFRC) {
                        ...
                        this.isCastling = true;
                    } else if ((cb.pieces[cb.colorToMove][4] & Util.POWER_LOOKUP[this.toIndex]) != 0L) {
                        if (this.toIndex == cb.castlingConfig.from_SquareID_rook_kingside_w) {
                            this.isCastling = true;
                            this.toIndex = 1;
                        } else if (this.toIndex == cb.castlingConfig.from_SquareID_rook_queenside_w) {
                            this.isCastling = true;
                            this.toIndex = 5;
                        } else if (this.toIndex == cb.castlingConfig.from_SquareID_rook_kingside_b) {
                            this.isCastling = true;
                            this.toIndex = 57;
                        } else {
                            if (this.toIndex != cb.castlingConfig.from_SquareID_rook_queenside_b) throw new IllegalStateException("King move is castling and king goes to a rook, which is not on the initial square.");
                            this.isCastling = true;
                            this.toIndex = 61;
                        }
                    } else {
                ...
                if (this.isCastling) {
                    ...
                    if (!cb.isValidMove(this.move)) throw new IllegalStateException("Not valid castling move");
                    return;
                }
