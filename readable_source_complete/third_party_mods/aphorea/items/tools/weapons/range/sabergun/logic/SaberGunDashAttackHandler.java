...
public class SaberGunDashAttackHandler
extends MousePositionAttackHandler {
            ...
            this.hudDrawElement = this.attackerMob.getLevel().hudManager.addElement(new HudDrawElement(){
                ...
                public void addDrawables(List<SortedDrawable> list, GameCamera camera, PlayerMob perspective) {
                    if (SaberGunDashAttackHandler.this.attackerMob.getAttackHandler() != SaberGunDashAttackHandler.this) {
                        ...
                        if (distance > 0.0f) {
                            Point2D.Float dir = GameMath.normalize((float)((float)SaberGunDashAttackHandler.this.lastX - SaberGunDashAttackHandler.this.attackerMob.x), (float)((float)SaberGunDashAttackHandler.this.lastY - SaberGunDashAttackHandler.this.attackerMob.y));
                            final DrawOptions drawOptions = HUD.getArrowHitboxIndicator((float)SaberGunDashAttackHandler.this.attackerMob.x, (float)SaberGunDashAttackHandler.this.attackerMob.y, (float)dir.x, (float)dir.y, (int)((int)distance), (int)50, (Color)new Color(0, 0, 0, 0), (Color)new Color(220, 255, 255, 100), (Color)new Color(0, 0, 0, 100), (GameCamera)camera);
    ...
    public void onEndAttack(boolean bySelf) {
        ...
        if (!this.endedByInteract && chargePercent >= 0.5f) {
            ...
            GameDamage damage = new GameDamage(DamageTypeRegistry.MELEE, baseDamage.damage * this.saberItem.getDashDamageMultiplier(this.item), baseDamage.armorPen, baseDamage.baseCritChance, baseDamage.playerDamageMultiplier, baseDamage.finalDamageMultiplier);
            Object event = this.attackerMob.buffManager.hasBuff("ninjascarf") && !this.attackerMob.getLevel().isTrialRoom ? new SaberJumpLevelEvent((Mob)this.attackerMob, this.seed, dir.x, dir.y, this.getChargeDistance(chargePercent), (int)(200.0f * chargePercent), damage) : new SaberDashLevelEvent((Mob)this.attackerMob, this.seed, dir.x, dir.y, this.getChargeDistance(chargePercent), (int)(200.0f * chargePercent), damage);
