...
public class IronSaber
extends AphSaberToolItem {
    public IronSaber() {
        super(300);
        this.rarity = Item.Rarity.NORMAL;
        this.attackDamage.setBaseValue(20.0f).setUpgradedValue(1.0f, 93.0f);
    ...
    public Projectile getProjectile(Level level, int x, int y, int targetX, int targetY, ItemAttackerMob attackerMob, InventoryItem item, float powerPercent, int seed) {
        return new AircutProjectile.IronAircutProjectile(level, (Mob)attackerMob, x, y, targetX, targetY, 200.0f * powerPercent, (int)(400.0f * powerPercent), this.getAttackDamage(item).modDamage(powerPercent), (int)((float)this.getKnockback(item, (Attacker)attackerMob) * powerPercent));
    }
}
