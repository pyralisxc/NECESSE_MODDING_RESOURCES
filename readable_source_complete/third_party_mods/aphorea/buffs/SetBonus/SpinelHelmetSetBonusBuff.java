...
public class SpinelHelmetSetBonusBuff
extends SetBonusBuff {
    ...
    public ListGameTooltips getTooltip(ActiveBuff ab, GameBlackboard blackboard) {
        ...
        tooltips.add((Object)new StringTooltips(Localization.translate((String)"itemtooltip", (String)"spinelhelmetsetbonus", (String)"healing", (String)AphMagicHealing.getMagicHealingToolTipPercent(ab.owner, ab.owner, 0.01f))));
        return tooltips;
    }

    public void onBeforeAttacked(ActiveBuff buff, MobBeforeHitEvent event) {
        super.onBeforeAttacked(buff, event);
        if (!event.isPrevented() && event.damage.type == DamageTypeRegistry.MELEE && event.damage.damage > 0.0f && event.target != null && event.target.isHostile) {
