...
public class UnstableGelArrowProjectile
extends Projectile {
    ToolItem toolItem;
    InventoryItem item;
    ...
    public AphAreaList getAreaList(GameDamage gameDamage) {
        return new AphAreaList(new AphArea(75.0f, this.color).setDamageArea(gameDamage));
    }
    ...
    public void dropItem() {
        if (GameRandom.globalRandom.getChance(0.5f)) {
            this.getLevel().entityManager.pickups.add((Entity)new InventoryItem("gelarrow").getPickupEntity(this.getLevel(), this.x, this.y));
        }
    }
