...
public class OpenLostUmbrellaProjectile
extends Projectile {
    ...
    public void addDrawables(List<LevelSortedDrawable> list, OrderableDrawables tileList, OrderableDrawables topList, OrderableDrawables overlayList, Level level, TickManager tickManager, GameCamera camera, PlayerMob perspective) {
        ...
        TextureDrawOptionsEnd options = this.texture.initDraw().light(light).rotate(this.getAngle() + 45.0f, this.texture.getWidth() / 2, 2).alpha(this.traveledDistance / (float)this.distance * 0.4f + 0.4f).pos(drawX, drawY - (int)this.getHeight());
