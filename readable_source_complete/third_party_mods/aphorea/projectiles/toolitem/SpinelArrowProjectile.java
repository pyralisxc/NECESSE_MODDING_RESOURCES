...
extends Projectile {
    ...
    public float tickMovement(float delta) {
        ...
        if (Double.isNaN(totalDist = Math.sqrt((moveX = this.getMoveDist(this.dx * speed, delta)) * moveX + (moveY = this.getMoveDist(this.dy * speed, delta)) * moveY)) || Double.isInfinite(totalDist)) {
            totalDist = 0.0;
        }
        this.moveDist(totalDist);
        if (this.removeIfOutOfBounds && (this.getX() < -100 || this.getY() < -100 || this.getX() > this.getLevel().tileWidth * 32 + 100 || this.getY() > this.getLevel().tileHeight * 32 + 100)) {
            this.remove();
        }
